...
class WaicWorkflowModel extends WaicModel {
	...
	/**
	 * Flush workflow caches when flows are published/unpublished.
	 */
	private function invalidateWorkflowCaches() {
		wp_cache_delete( self::CACHE_KEY_HOOKED );
		wp_cache_delete( self::CACHE_KEY_SCHEDULED );
		wp_cache_delete( self::CACHE_KEY_WEBHOOK );
	}
	
	public function existScheduledFlows() {
		$cached = wp_cache_get( self::CACHE_KEY_SCHEDULED );
		if ( false !== $cached ) {
			return $cached;
		}
		$exists = WaicDb::get('SELECT 1 FROM `@__workflows` WHERE status=1 AND tr_type=1 LIMIT 1', 'one');
		$result = $exists ? 1 : 0;
		wp_cache_set( self::CACHE_KEY_SCHEDULED, $result, '', self::CACHE_TTL );
		return $result;
	}
	...
	/**
	 * Get all external block paths registered by plugins via filter.
	 * Each plugin can provide its own blocks/ directory.
	 *
	 * @return array Array of absolute directory paths.
	 */
	public function getExternalBlocksPaths() {
		if (is_null($this->_externalPaths)) {
			$paths = WaicDispatcher::applyFilters('getExternalBlocksPaths', array());
			$this->_externalPaths = array();
			if (!empty($paths) && is_array($paths)) {
				foreach ($paths as $p) {
					if (!empty($p) && is_dir($p)) {
						$this->_externalPaths[] = rtrim($p, '/\\') . WAIC_DS;
					}
				}
			}
		}
		return $this->_externalPaths;
	}
	private function loadAllBlocks() {
		$this->_blCodes = array();
		$pathes = array_merge(
			array($this->getCustomBlocksPath()),
			$this->getExternalBlocksPaths(),
			array($this->getBlocksPath())
		);
	...
	public function getBlockClass( $typ, $code ) {
		...
		if (!isset($_blockClasses[$typ][$code])) {
			...
			$file = $typ . 's/' . $code . '.php';
			$found = false;
			// 1. Try custom blocks path (legacy single global option)
			$custom = $this->getCustomBlocksPath();
			if ($custom) {
				$found = waicImportClass($name, $custom . $file);
			}
			// 2. Try external plugin paths (multi-provider)
			if (!$found) {
				foreach ($this->getExternalBlocksPaths() as $extPath) {
					$found = waicImportClass($name, $extPath . $file);
					if ($found) break;
				}
			}
			// 3. Fall back to built-in blocks path
			if (!$found) {
		...
		if (is_null($this->_blCategories)) {
			$this->_blCategories = array(
				'triggers' => array(
					'un' => array(),
					'wu' => array(
						'name' => __('Sự kiện tin nhắn', 'ai-copilot-content-generator'),
						'desc' => __('Hotline của BizCity, Zalo Bot, Facebook Messenger,...', 'ai-copilot-content-generator'),
					),
					'sy' => array(
						'name' => __('Sự kiện từ hệ thống', 'ai-copilot-content-generator'),
						'desc' => __('Lên lịch, Webhooks, đường dẫn, ...', 'ai-copilot-content-generator'),
					),
					'wp' => array(
						'name' => __('Tự kiện từ WordPress', 'ai-copilot-content-generator'),
						'desc' => __('Đăng ký người dùng, xuất bản bài viết, gửi bình luận, v.v.', 'ai-copilot-content-generator'),
					),
					'wc' => array(
						'name' => __('Sự kiện WooCommerce', 'ai-copilot-content-generator'),
						'desc' => __('Đơn hàng mới, hoàn tất thanh toán, thay đổi tồn kho, v.v.', 'ai-copilot-content-generator'),
					),
				...
				'actions' => array(
					'un' => array(),
					'ai' => array(
						'name' => __('Hành động AI', 'ai-copilot-content-generator'),
						'desc' => __('Phân tích tình huống, lập kế hoạch, tạo bài viết, sản phẩm, ...', 'ai-copilot-content-generator'),
					),
					'wp' => array(
						'name' => __('Hành động được đóng gói', 'ai-copilot-content-generator'),
						'desc' => __('Gửi Zalo, gửi email, tạo bài viết, cập nhật người dùng, quản lý nội dung', 'ai-copilot-content-generator'),
					),
					'wc' => array(
						'name' => __('Hành động WooCommerce', 'ai-copilot-content-generator'),
						'desc' => __('Quản lý đơn hàng, sản phẩm và khách hàng', 'ai-copilot-content-generator'),
					),
					'kling' => array(
						'name' => __('Tạo Video AI', 'ai-copilot-content-generator'),
						'desc' => __('Tạo video AI với BizVideo', 'ai-copilot-content-generator'),
					),
					'em' => array(
						'name' => __('Hành động email', 'ai-copilot-content-generator'),
						'desc' => __('Gửi email, đọc email, đang phát triển ...', 'ai-copilot-content-generator'),
					),
					'ca' => array(
						'name' => __('Lịch & Cuộc họp', 'ai-copilot-content-generator'),
						'desc' => __('Tạo sự kiện lịch và cuộc họp', 'ai-copilot-content-generator'),
					),
					'te' => array(
						'name' => __('Hành động Telegram', 'ai-copilot-content-generator'),
						'desc' => __('Bị chặn, đang phát triển', 'ai-copilot-content-generator'),
					),
					/*
					'sl' => array(
						'name' => __('Hành động Slack', 'ai-copilot-content-generator'),
						'desc' => __('Gửi tin nhắn Slack', 'ai-copilot-content-generator'),
					),
					
					'di' => array(
						'name' => __('Hành động Discord', 'ai-copilot-content-generator'),
						'desc' => __('Gửi tin nhắn và nhúng đến các kênh Discord', 'ai-copilot-content-generator'),
					),
					'db' => array(
						'name' => __('Hành động Database', 'ai-copilot-content-generator'),
						'desc' => __('Thực thi các truy vấn SQL', 'ai-copilot-content-generator'),
					),*/
				),
				'logics' => array(
					'lp' => array(
						'name' => __('Các vòng lặp', 'ai-copilot-content-generator'),
						'desc' => __('Xử lý nhiều mục một cách tuần tự', 'ai-copilot-content-generator'),
					),
					'un' => array(),
				),
			);
	...
	public function getWorkflowSettings() {
		...
		$settings = array(
			'timeout' => array(
				'type' => 'number',
				'label' => __('Thời gian tối đa cho phép để hoàn thành workflow (giây)', 'ai-copilot-content-generator'),
				'default' => 300,
				'min' => 10,
			),
			'multiple' => array(
				'type' => 'select',
				'label' => __('Cho phép cùng một trigger chạy nhiều lần', 'ai-copilot-content-generator'),
				'default' => 0,
				'options' => $yesNo,
			),
			'cooldown' => array(
				'type' => 'number',
				'label' => __('Thời gian chờ giữa các lần chạy', 'ai-copilot-content-generator'),
				'default' => 0,
				'add' => array('units'),
			),
			'units' => array(
			...
			'skip' => array(
				'type' => 'select',
				'label' => __('Bỏ qua nếu đang chạy', 'ai-copilot-content-generator'),
				'default' => 0,
				'options' => $yesNo,
			),
			'stop' => array(
				'type' => 'select',
				'label' => __('Dừng workflow khi có lỗi', 'ai-copilot-content-generator'),
				'default' => 'yes',
				'options' => array(
			...
			'send' => array(
				'type' => 'select',
				'label' => __('Gửi thông báo lỗi', 'ai-copilot-content-generator'),
				'default' => 0,
				'options' => $yesNo,
			),
			'ai_all' => array(
				'type' => 'number',
				'label' => __('Tổng số yêu cầu AI tối đa', 'ai-copilot-content-generator'),
				'default' => 0,
				'min' => 0,
			),
			'ai_run' => array(
				'type' => 'number',
				'label' => __('Tổng số yêu cầu AI tối đa mỗi lần chạy', 'ai-copilot-content-generator'),
				'default' => 0,
				'min' => 0,
			),
		);
	...
	public function controlTaskParameters( $params, &$error = '', &$errNodes = array(), $taskId = 0 ) {
		...
		foreach ($nodes as $node) {
			...
			if (empty($nodeData)) {
				$error = __('Nút nguồn [' . $nodeId . '] có cấu trúc bị hỏng.', 'ai-copilot-content-generator');
				return false;
			}
			...
			if (count($startEdges) > 1) {
				$error = __('Nút nguồn [' . $nodeId . '] chỉ có thể được liên kết với một đích.', 'ai-copilot-content-generator');
				return false;
			}
			...
			if (count($elseEdges) > 1) {
				$error = __('Nút nguồn [' . $nodeId . '] từ mỗi đầu ra chỉ có thể được kết nối với một đích.', 'ai-copilot-content-generator');
				return false;
			}
			...
			if (count($thenEdges) > 1) {
				$error = __('Nút nguồn [' . $nodeId . '] từ mỗi đầu ra chỉ có thể được liên kết với một đích.', 'ai-copilot-content-generator');
				return false;
			}
			if ($this->hasPath($nodeId, $nodeId, $edges)) {
				$error = __('Nút nguồn [' . $nodeId . '] bị lặp hoặc trong vòng lặp.', 'ai-copilot-content-generator');
				return false;
			}
					...
					if (!empty($matches[1])) {
						foreach ($matches[1] as $nId) {
							if (!$this->hasPath($nId, $nodeId, $edges)) {
								...
								$error = __('Nút nguồn [' . $nodeId . '] có biến không hợp lệ - tham chiếu đến nút không tồn tại.', 'ai-copilot-content-generator');
								return false;
							}
						}
					}
		...
		foreach ($loopNodes as $loopId => $children) {
			foreach ($children as $nodeId => $child) {
				if (isset($loopNodes[$child])) {
					$error = __('Không cho phép vòng lặp lồng nhau [' . $child . '].', 'ai-copilot-content-generator');
					return false;
				}
				foreach ($loopNodes as $lpId => $chs) {
					if ($lpId != $loopId && in_array($child, $chs)) {
						$error = __('Nhánh Vòng lặp phải được cách ly và không có kết nối với các nút bên ngoài nhánh. Nút gặp sự cố [' . $child . ']', 'ai-copilot-content-generator');
						return false;
					}
				}
				foreach ($endNodes as $lpId => $chs) {
					if (in_array($child, $chs)) {
						$error = __('Nhánh Vòng lặp phải được cách ly và các nút bên trong vòng lặp không nên có khả năng đến đầu ra END. Nút gặp sự cố [' . $child . ']', 'ai-copilot-content-generator');
						return false;
					}
				}
			}
	...
	public function convertTaskParameters( $params, $toDB = true ) {
		...
		if ($toDB) {
			// When saving to DB, clear error field from all nodes
			foreach ($nodes as $n => $node) {
				if (isset($nodes[$n]['data']['error'])) {
					unset($nodes[$n]['data']['error']);
				}
			}
			$params['nodes'] = $nodes;
		} else {
			// When loading from DB, decode HTML entities
			foreach ($nodes as $n => $node) {
	...
	public function isRunningFlow() {
		...
		if (is_null($option) || empty($option)) {
			// No flag record → not running (safe default)
			return false;
		}
		...
		if ($option['flag'] + $option['timeout'] < WaicUtils::getTimestamp()) {
				error_log('[WORKFLOW] isRunningFlow: timeout expired, stopping runId=' . $option['value']);
				$this->runModel->stopRun((int) $option['value'], 6, 'Stopped by timeout');
				$this->workspace->resetRunningFlag($this->runningFlowId);
			return false;
		}
		error_log('[WORKFLOW] isRunningFlow: TRUE (runId=' . $option['value'] . ' age=' . (WaicUtils::getTimestamp() - $option['flag']) . 's timeout=' . $option['timeout'] . 's)');
		return true;
	}
	...
	/**
	 * Maximum workflow runs to process per cron tick.
	 * Prevents runaway CPU when queue is large.
	 * @since 1.4.5 — batch limit added (was unlimited while-true loop)
	 */
	const MAX_BATCH_RUNS = 10;
	...
	public function doFlowRuns() {
		...
		// Allow enough time for the batch but NOT unlimited (was set_time_limit(0))
		set_time_limit(120);
		$processed = 0;
		do {
			if ($processed >= self::MAX_BATCH_RUNS) {
				break; // Batch limit reached — remaining runs will be picked up next cron tick
			}
	...
	public function doFlowRun( $run ) {
		...
		if (!$this->setRunningFlow(array('value' => $runId, 'flag' => WaicUtils::getTimestamp(), 'timeout' => empty($timeout) ? $this->runningTimeout : $timeout))) {
			error_log('[WORKFLOW] doFlowRun: setRunningFlow FAILED for runId=' . $runId);
			return false;
		}
		error_log('[WORKFLOW] doFlowRun: setRunningFlow OK, runId=' . $runId);
		...
		error_log('[WORKFLOW] doFlowRun: trId=' . $trId . ' nodes=' . count($nodes) . ' edges=' . count($edges) . ' runId=' . $runId . ' logId=' . $logId);
		error_log('[WORKFLOW] doFlowRun: edges=' . wp_json_encode($edges));
		$prevNodeId = $trId;
		$curNodeId = 0;
		$curNode = false;
		...
		do {
			if (empty($curNodeId)) {
				...
				error_log('[WORKFLOW] doFlowRun LOOP: prevNodeId=' . $prevNodeId . ' sourceHandle=' . $sourceHandle . ' => curNodeId=' . var_export($curNodeId, true));
			}
			...
			if (empty($node)) {
				WaicFrame::_()->pushError('Nút nguồn với id=' . $curNodeId . ' không tìm thấy');
				return 7;
			}
			$block = $this->getFlowBlock($node);
			error_log('[WORKFLOW] doFlowRun BLOCK: curNodeId=' . $curNodeId . ' block=' . ($block ? get_class($block) : 'FALSE') . ' nodeData=' . wp_json_encode(WaicUtils::getArrayValue($node, 'data', [], 2)));
			if ($block) {
				...
				// ⭐ CRITICAL: Set runId for HIL/delay blocks to enable resume
				if (method_exists($block, 'setRunId')) {
					$block->setRunId($runId);
				}
				...
				if ($saveToken && !empty($results['tokens'])) {
					...
					if ($maxAIRun && $runTokens > $maxAIRun) {
						WaicFrame::_()->pushError('Dừng bởi giới hạn token chạy (' . $runTokens . ' > ' . $maxAIRun . ')');
						return 6;
					}
					if ($maxAIAll) {
						$flowTokens += $results['tokens'];
						if ($flowTokens > $maxAIAll) {
							WaicFrame::_()->pushError('Dừng bởi giới hạn token luồng (' . $flowTokens . ' > ' . $maxAIAll . ')');
							if (1 == $flow['status']) {
								$this->updateById(array('status' => 6, 'updated' => WaicUtils::getTimestampDB()), $flId);
							}
							return 6;
				...
				WaicFrame::_()->pushError('Block với code=' . $block['code'] . ' không tìm thấy');
				return 7;
			}
		...
		error_log('[WORKFLOW] doFlowRun: COMPLETED runId=' . $runId);
		return true;
	}
	...
	/**
	 * Cache key for hooked workflows list.
	 * Invalidated in publishFlows() / unpublishFlows().
	 */
	const CACHE_KEY_HOOKED = 'waic_hooked_flows';
	const CACHE_KEY_SCHEDULED = 'waic_exist_scheduled';
	const CACHE_KEY_WEBHOOK = 'waic_webhook_flows';
	const CACHE_TTL = 3600; // 1 hour

	public function doHookedFlows( $taskId = 0 ) {
		// Prevent double-registration: init@20 + bizcity_aiwu_fire_twf_process_flow
		static $booted = false;
		if ($booted) return;
		$booted = true;

		$flows = wp_cache_get( self::CACHE_KEY_HOOKED );
		if ( false === $flows ) {
			$flows = $this->setSelectFields('id, tr_hook')->setWhere(array('status' => 1, 'tr_type' => 2))->getFromTbl();
			wp_cache_set( self::CACHE_KEY_HOOKED, $flows ?: array(), '', self::CACHE_TTL );
		}
		if ($flows) {
			foreach ( $flows as $flow ) {
				$flowId = $flow['id'];
			...
			if ($node) {
				...
				if ($block) {
					...
					if ( ! empty( $result ) ) {
						...
						$skipResult = $this->needSkip($flow['flags'], $flId, $objId, $flow['sch_period']);
						error_log('[WORKFLOW] needSkip: ' . ($skipResult ? 'YES' : 'FALSE') . ' ('. ($objId ? 'objId='.$objId : 'no objId') . ') flowId=' . $flId . ' objId=' . $objId);
						if ($skipResult) {
							return;
						}
						$runId = $this->getModule()->getModel('flowruns')->createRun($flow['task_id'], $flId, $result, $objId);
						error_log('[WORKFLOW] createRun flId=' . $flId . ' runId=' . ($runId ?: 'FAILED') . ' taskId=' . $flow['task_id']);
						if (!empty($runId)) {
							...
							if (!empty($flow['tr_hook']) && $flow['tr_hook'] === 'waic_twf_process_flow') {
								...
								error_log('[WORKFLOW] TWF sync: about to processRunNow runId=' . $runId);
								$runSync = apply_filters('waic_twf_process_flow_run_sync', true, $flow, (int) $runId, $result, $args);
								if ($runSync) {
									$syncResult = $this->processRunNow((int) $runId);
									error_log('[WORKFLOW] TWF sync: processRunNow result=' . ($syncResult ? 'OK' : 'BLOCKED'));
								} else {
									error_log('[WORKFLOW] TWF sync DISABLED by filter');
								}
							}
						} else {
							error_log('[WORKFLOW] createRun FAILED for flId=' . $flId);
						}
					} else {
						error_log('[WORKFLOW] controlRun REJECTED for flId=' . $flId . ' taskId=' . $flow['task_id'] . ' — trigger filter did not match');
					}
				}
			}
		}
	...
	private function processRunNow( $runId ) {
		$runId = (int) $runId;
		if (empty($runId)) {
			error_log('[WORKFLOW] processRunNow: empty runId');
			return false;
		}
		$this->workspace = WaicFrame::_()->getModule('workspace')->getModel();
		$this->runModel = $this->getModule()->getModel('flowruns');
		...
		if ($this->isRunningFlow()) {
			error_log('[WORKFLOW] processRunNow: runId=' . $runId . ' QUEUED (another flow running)');
			return false;
		}

		$run = $this->runModel->getById($runId);
		if (empty($run) || (int) $run['status'] !== 1) {
			error_log('[WORKFLOW] processRunNow: runId=' . $runId . ' SKIP (status=' . ($run['status'] ?? 'null') . ')');
			return false;
		}
		error_log('[WORKFLOW] processRunNow: runId=' . $runId . ' EXECUTING inline');

		try {
			$result = $this->doFlowRun($run);
			error_log('[WORKFLOW] processRunNow: doFlowRun returned=' . var_export($result, true));
			if (false === $result) {
				return false;
			}
			if (7 === $result || 6 === $result) {
	...
	public function needSkip( $flags, $flowId, $objId = 0, $period = 0 ) {
		...
		if ($flag) {
			...
			if ($skip) {
				error_log('[WORKFLOW] needSkip: TRUE (existRunningRuns) flowId=' . $flowId);
				return true;
			}
		}
		if (!$skip && !empty($objId)) {
			...
			// ===== HIL BYPASS: Allow multiple runs for HIL workflows =====
			global $waic_current_trigger;
			$isHilRun = false;
			if (is_array($waic_current_trigger)) {
				$isHilRun = !empty($waic_current_trigger['hil_resume']) || 
				           !empty($waic_current_trigger['hil_completed']) ||
				           strpos((string)($waic_current_trigger['obj_id'] ?? ''), '_hil_') !== false;
			}
			
			if ($isHilRun) {
				error_log('[WORKFLOW] HIL run detected, bypassing needSkip for obj_id=' . $objId);
				return false; // Force allow HIL runs
			}
			...
			if (!$multiple || !empty($period)) {
				...
				if ($lastRun && !empty($lastRun)) {
					...
					error_log('[WORKFLOW] needSkip: ' . ($skip ? 'TRUE' : 'FALSE') . ' (multiple check) flowId=' . $flowId . ' objId=' . $objId . ' multiple=' . $multiple . ' lastRun=' . wp_json_encode($lastRun));
				} else {
					error_log('[WORKFLOW] needSkip: FALSE (no lastRun) flowId=' . $flowId . ' objId=' . $objId);
				}
			} else {
				error_log('[WORKFLOW] needSkip: FALSE (multiple allowed) flowId=' . $flowId . ' objId=' . $objId . ' multiple=' . $multiple);
			}
		} else {
			error_log('[WORKFLOW] needSkip: FALSE (no objId) flowId=' . $flowId . ' objId=' . $objId);
		}
		return $skip;
	}
	...
	public function registerWebhookRoutes() {
		$flows = wp_cache_get( self::CACHE_KEY_WEBHOOK );
		if ( false === $flows ) {
			$flows = $this->setSelectFields('id, tr_hook')->setWhere(array('status' => 1, 'tr_type' => 3))->getFromTbl();
			wp_cache_set( self::CACHE_KEY_WEBHOOK, $flows ?: array(), '', self::CACHE_TTL );
		}
		if ($flows) {
			foreach ($flows as $flow) {
				$flId = $flow['id'];
	...
	public function canAccessWebhook( WP_REST_Request $request ) {
		...
		// Check for webhook secret token in header or query param
		$expected = get_option('bizcity_webhook_secret', '');
		if (!empty($expected)) {
			$token = $request->get_header('X-Webhook-Token');
			if (empty($token)) {
				$token = $request->get_param('webhook_token');
			}
		...
		// Fallback: if no secret configured, require admin login
    ...
    private function bizcity_guess_last_run_id() {
        global $wpdb;
        $table = $wpdb->prefix . WAIC_DB_PREF . 'flowruns';
