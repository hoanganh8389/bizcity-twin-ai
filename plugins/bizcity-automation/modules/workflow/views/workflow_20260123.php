...
class WaicWorkflowView extends WaicView {
	...
	public function showWorkflow() {
		...
		$frame->addScript('waic-workflow', $path . 'admin.workflow.js');
		$frame->addStyle('waic-workflow', $path . 'admin.workflow.css');
		$frame->addScript('waic-workflow-import', $path . 'admin.workflow.import.js');
		$frame->addScript('waic-workflow-export', $path . 'admin.workflow.export.js');
	...
	public function showWorkflowBuilder( $taskId = 0 ) {
        ...
        // Builder bundle
        $frame->addScript('waic-workflow-editor', $path . 'workflow.editor.js', array('jquery','wp-element','wp-hooks'));
        $frame->addStyle('waic-workflow-editor', $path . 'workflow.editor.css');

        // BizCity: also load admin.workflow.js/css in builder (for repeater UI enhancement)
        $frame->addScript('waic-workflow', $path . 'admin.workflow.js', array('jquery'));
        $frame->addStyle('waic-workflow', $path . 'admin.workflow.css');
		...
		$jsData = array(
			...
			'lang' => array(
				...
				'btn_run' => __('Run', 'ai-copilot-content-generator'),
				...
				'var_tooltip_webhook_field' => __('Enter Webhook Field Key. Separate keys with forward-slash / to build nested data.', 'ai-copilot-content-generator') . '</br></br>' . __('Example', 'ai-copilot-content-generator') . ':</br>user/id</br>address/street</br>description',
				'var_plh_webhook_result' => __('Enter Field Key', 'ai-copilot-content-generator'),
				'var_tooltip_webhook_result' => __('Enter Webhook Result Field Key. Separate keys with forward-slash / to build nested data.', 'ai-copilot-content-generator') . '</br></br>' . __('Example', 'ai-copilot-content-generator') . ':</br>code</br>message</br>data/status',
