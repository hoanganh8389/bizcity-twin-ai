(function($, app) {
	...
	WorkflowPage.prototype.initHistoryTable = function () {
		...
		_this.waicHistoryTableObj = _this.waicHistoryTable.DataTable({
			...
			processing: true,
			ajax: {
				'url': '/wp-json/waic/v1/workflow/history',
				'type': 'POST',
				'beforeSend': function (xhr) {
					xhr.setRequestHeader('X-WP-Nonce', WAIC_DATA.wpRestNonce);
				},
				data: function (d) {
...
(function ($, app) {
  ...
  function getKeyEventTarget() {
    // Prefer ReactFlow pane so workflow.editor.js sees a real Element target (hasAttribute exists)
    return (
      document.querySelector(".react-flow__pane") ||
      document.querySelector(".react-flow") ||
      document.querySelector("#waic-workflow-root") ||
      document.body
    );
  }
  ...
  function focusCanvas() {
    ...
    if (pane) {
      // Make focusable (ReactFlow pane is often a div)
  ...
  function fireKey(key, code) {
    const target = getKeyEventTarget();
    ...
    try {
      ...
      // Avoid noisy uncaught errors if browser blocks synthetic events in some cases
      // (still keep fallback delete-by-save-interceptor working)
      // console.warn("[WAIC] fireKey failed:", e);
    }
  }

  function tryDeleteViaKeys() {
    focusCanvas();
    // Prefer Backspace (builder often uses it), then Delete
  ...
  function injectButtons() {
    ...
    btnDelete.className = "wbw-button-small waic-flow-delete-selected";
    btnDelete.innerHTML =
      '<span class="dashicons dashicons-trash" style="vertical-align:middle;margin-right:6px;"></span>Chọn "mũi tên" hoặc "khối" và nhấn để xóa';
