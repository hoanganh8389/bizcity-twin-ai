...
class WaicAction_ca_update_meeting extends WaicAction {
	protected $_code = 'ca_update_meeting';
	protected $_order = 3;
	
	public function __construct( $block = null ) {
		$this->_name = __('Update Meeting', 'ai-copilot-content-generator');
		...
		if ($integration) {
			...
			if (empty($meetingId)) {
				...
				if (!empty($start) && !WaicUtils::checkDateTimeFormat($start, 'Y-m-d H:i')) {
					...
					$result = $integration->doUpdateMeeting($data);
