<?php
/**
 * BizCity Tool Content — Custom Post Type Registration
 *
 * Replaces custom DB tables with WordPress CPT + wp_postmeta.
 */
if ( ! defined( 'ABSPATH' ) ) exit;

class BizCity_Content_Post_Type {

    const POST_TYPE = 'bza-content';
    ...
    public static function register() {
        register_post_type( self::POST_TYPE, [
            'labels' => [
                'name'          => 'Agent Content History',
                'singular_name' => 'Agent Content Entry',
            ],
            'public'          => false,
            'show_ui'         => false,
            'show_in_rest'    => false,
     ...
     */
    public static function save_history( int $user_id, string $goal, string $prompt, string $ai_title, string $ai_content, ?int $result_id, string $result_url, string $image_url ) {
    ...
    public static function get_history( int $user_id, int $limit = 30 ): array {
        ...
        foreach ( $query->posts as $p ) {
            $items[] = (object) [
                ...
                'post_id'    => get_post_meta( $p->ID, '_bza_result_id', true ),
                'post_url'   => get_post_meta( $p->ID, '_bza_result_url', true ),
