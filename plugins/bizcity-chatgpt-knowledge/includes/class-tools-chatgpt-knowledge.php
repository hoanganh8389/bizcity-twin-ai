<?php
/**
 * BizCity ChatGPT Knowledge — Tool Callbacks
 *
...
class BizCity_Tools_ChatGPT_Knowledge {

    /** Plugin display name for personality prefix */
    const PLUGIN_LABEL = 'ChatGPT Knowledge';

    /* ══════════════════════════════════════════════════════
     *  TOOL: ask_chatgpt — Hỏi ChatGPT kiến thức chuyên sâu
     *
     ...
     *
     *  Personality inject:
     *    "Mình dùng công cụ ChatGPT Knowledge để tìm hiểu
     ...
     */
    public static function ask_chatgpt( array $slots ): array {
        ...
        if ( ! class_exists( 'BizCity_ChatGPT_Knowledge' ) ) {
            return [
                'success'  => false,
                'complete' => true,
                'message'  => '⚠️ ChatGPT Knowledge chưa được kích hoạt.',
                'data'     => [],
            ];
        }
        ...
        $result  = $chatgpt->ask( $question, $options );
        ...
        return [
            'success'  => false,
            'complete' => true,
            'message'  => '⚠️ ChatGPT không phản hồi: ' . ( $result['error'] ?? 'Unknown error' ),
            'data'     => [],
        ];
    }

    /* ══════════════════════════════════════════════════════
     *  TOOL: chatgpt_save_bookmark — Lưu bookmark câu trả lời
     * ══════════════════════════════════════════════════════ */
     ...
     * @return array Tool Output Envelope
     */
    public static function chatgpt_save_bookmark( array $slots ): array {
        ...
        global $wpdb;
        $table = $wpdb->prefix . 'bzck_bookmarks';
    ...
    /* ══════════════════════════════════════════════════════
     *  TOOL: chatgpt_search_history — Xem lịch sử tìm kiếm
     * ══════════════════════════════════════════════════════ */
     ...
     * @return array Tool Output Envelope
     */
    public static function chatgpt_search_history( array $slots ): array {
        $user_id = get_current_user_id();
        if ( ! $user_id ) {
            return [
                'success'  => false,
