...
function bzgk_get_knowledge_character_id() {
    return (int) get_option( 'bzgk_knowledge_character_id', 0 );
}

/* ─── Lấy knowledge context từ bizcity-knowledge ─── */
function bzgk_get_knowledge_context( $query, $max_tokens = 4000 ) {
    $char_id = bzgk_get_knowledge_character_id();
...
function bzgk_knowledge_settings_section() {
    $char_id    = bzgk_get_knowledge_character_id();
    $characters = bzgk_get_available_characters();
    ?>
    <h3>📚 Đào tạo kiến thức cho Knowledge AI</h3>
    <table class="form-table">
        <tr>
            <th>Character liên kết</th>
            <td>
                <select name="bzgk_knowledge_character_id">
                ...
                <p class="description">
                    Character chứa kiến thức RAG (FAQ, files, web crawl) cho Gemini Knowledge.
                    Kiến thức sẽ tự động inject vào system prompt khi Gemini xử lý câu hỏi knowledge.
                </p>
            </td>
        </tr>
    </table>
    ...
    if ( $char_id ) {
        ...
        $stats = bzgk_get_knowledge_stats( $char_id );
        if ( $stats ) {
...
function bzgk_get_available_characters() {
    global $wpdb;
...
function bzgk_get_knowledge_stats( $character_id ) {
    global $wpdb;
