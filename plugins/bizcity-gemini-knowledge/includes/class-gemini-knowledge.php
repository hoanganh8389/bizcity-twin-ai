<?php
/**
 * BizCity Gemini Knowledge — API Helper.
 *
 * Singleton class used for admin "Ask" page and shortcode.
 * Provides a simple ask() method and search logging.
 *
...
class BizCity_Gemini_Knowledge {
    ...
    /**
     * Ask Gemini a question (standalone — for admin Ask page / shortcode).
     *
    ...
    public function ask( $question, $options = [] ) {
        ...
        $model       = $options['model']       ?? $settings['model']       ?? BizCity_Gemini_Knowledge_Pipeline::MODEL_PRIMARY;
        $temperature = $options['temperature'] ?? $settings['temperature'] ?? BizCity_Gemini_Knowledge_Pipeline::TEMPERATURE;
        $max_tokens  = $options['max_tokens']  ?? $settings['max_tokens']  ?? BizCity_Gemini_Knowledge_Pipeline::MAX_TOKENS;

        // Build messages
        $system_prompt = "Bạn là trợ lý kiến thức AI chuyên sâu — Gemini Knowledge.\n"
        ...
        // RAG context
        $rag_ctx = bzgk_get_knowledge_context( $question, 3000 );
        if ( $rag_ctx ) {
            $system_prompt .= "\n\n## 📚 Kiến thức liên quan:\n" . $rag_ctx;
        }
        ...
        error_log( '[BZGK_ASK] v2 stream_detect: fn=' . (int) $fn_exists . ' hook=' . (int)(bool) $has_hook . ' cb=' . ( $stream_cb ? 1 : 0 ) );
        ...
        if ( $stream_cb && function_exists( 'bizcity_openrouter_chat_stream' ) ) {
            $result = bizcity_openrouter_chat_stream( $messages, [
                'model'       => $model,
                'purpose'     => 'gemini-knowledge-ask',
                'temperature' => floatval( $temperature ),
                'max_tokens'  => intval( $max_tokens ),
            ], $stream_cb );

            if ( empty( $result['success'] ) && $model === BizCity_Gemini_Knowledge_Pipeline::MODEL_PRIMARY ) {
                $result = bizcity_openrouter_chat_stream( $messages, [
                    'model'       => BizCity_Gemini_Knowledge_Pipeline::MODEL_FALLBACK,
                    'purpose'     => 'gemini-knowledge-ask-fallback',
                    'temperature' => floatval( $temperature ),
                    'max_tokens'  => intval( $max_tokens ),
                ], $stream_cb );
            }
        ...
        // Log search (both success and failures)
    ...
    /**
     * Log a search query to history table.
     */
    ...
    public function get_available_models() {
        return BizCity_Gemini_Knowledge_Pipeline::MODELS;
    }
    ...
    public function get_settings() {
        return get_option( 'bzgk_settings', [
