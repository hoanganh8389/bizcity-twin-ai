...
class BizCoach_Pro_Coach_Builder {
	...
	// Model used for JSON-schema structured generation (rich map sections).
	// Template generators can override per-key via "model" field.
	const JSON_GEN_MODEL      = 'anthropic/claude-3.5-haiku';
	const JSON_GEN_MAX_TOKENS = 4096;
	...
	public static function rest_generate_section( WP_REST_Request $req ) {
		@set_time_limit( 120 ); // LLM calls can be slow; prevent premature PHP kill → Cloudflare 502.
		...
		// Resolve full generator definition from BizCoach-Pro template registry
		// (file-based seed → may include output/render/json_schema/system fields
		// that the legacy bccm_coach_type_base_registry doesn't carry).
		$gen_def = self::find_generator_def( $coach_type, $gen_key );
		if ( $gen_def && $gen_label === $gen_key ) {
			$gen_label = (string) ( $gen_def['label'] ?? $gen_label );
		}
		$is_json_mode = $gen_def && ( ( $gen_def['output'] ?? '' ) === 'json' );
		...
		if ( ! $force && function_exists( 'bccm_get_gen_results' ) ) {
			...
			if ( $existing && ! empty( $existing['result_json'] ) && ( $existing['status'] ?? '' ) === 'success' ) {
				$cached_md   = self::extract_section_markdown( $existing['result_json'] );
				$cached_html = self::extract_section_html( $existing['result_json'] );
				if ( $cached_md !== '' || $cached_html !== '' ) {
					return rest_ensure_response( array(
						'ok'           => true,
						'gen_key'      => $gen_key,
						'label'        => $gen_label,
		...
		if ( $is_json_mode ) {
			$schema_str = wp_json_encode( $gen_def['json_schema'] ?? array(), JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT );
			$sys_extra  = (string) ( $gen_def['system'] ?? '' );
			$sys = ( $sys_extra !== '' ? $sys_extra : "Bạn là **{$coach_label}** — huấn luyện viên chuyên sâu. Phân tích phần \"{$gen_label}\"." ) .
				"\n\nQUY TẮC OUTPUT (BẮT BUỘC):\n" .
				"- Trả về DUY NHẤT một đối tượng JSON hợp lệ (RFC 8259), KHÔNG markdown, KHÔNG code-fence, KHÔNG lời chào.\n" .
				"- Tuân thủ nghiêm ngặt JSON Schema sau (mọi field `required` PHẢI có; respect `enum`, `minimum`, `maximum`, `minItems`, `maxItems`):\n" .
				"```json\n{$schema_str}\n```\n" .
				"- Tiếng Việt tự nhiên, gọi đích danh tên người dùng nếu có.";
		} else {
		...
		if ( empty( $resp['success'] ) ) {
			return new WP_REST_Response( array( 'error' => 'llm_failed', 'detail' => $resp['error'] ?? '' ), 422 );
		}
		$raw = trim( (string) ( $resp['message'] ?? '' ) );
		if ( $raw === '' ) {
			return new WP_REST_Response( array( 'error' => 'empty_response' ), 422 );
		}

		$envelope = array( 'generated_by' => 'bcpro_coach_builder' );
		$content_md   = '';
		$content_html = '';

		if ( $is_json_mode ) {
			$json = self::parse_json_response( $raw );
			if ( ! is_array( $json ) ) {
				// Fallback: persist raw as markdown so user still sees something.
				$content_md         = $raw;
				$envelope['content_md']    = $content_md;
				$envelope['parse_error']   = 'invalid_json';
			} else {
				$render_name           = (string) ( $gen_def['render'] ?? '' );
				$content_html          = BCPro_Section_Renderer::render( $render_name, $json );
				$envelope['content_json']   = $json;
				$envelope['content_html']   = $content_html;
				$envelope['render']         = $render_name;
			}
		} else {
			// Markdown mode — strip wrapping code-fence if model added one.
			$md = $raw;
			if ( preg_match( '/^```(?:markdown|md)?\s*(.+)```\s*$/is', $md, $m ) ) { $md = trim( $m[1] ); }
			$content_md            = $md;
			$envelope['content_md'] = $md;
		}
	...
	public static function rest_section_status( WP_REST_Request $req ) {
		...
		foreach ( $gens as $g ) {
			...
			if ( $has ) {
				...
				$html = self::extract_section_html( $row['result_json'] );
				if ( $md   !== '' ) { $item['content_md']   = $md; }
				if ( $html !== '' ) { $item['content_html'] = $html; }
			}
			$out[] = $item;
		}
	...
	public static function extract_section_markdown( $json_str ) {
		...
		// JSON-mode envelope already has content_html — no markdown fallback needed
		// (avoid shipping the whole envelope back to client).
		if ( ! empty( $dec['content_html'] ) || ! empty( $dec['content_json'] ) ) {
			return '';
		}
		// Truly legacy structured shape (no envelope keys) — render as JSON code-block fallback.
		return "```json\n" . wp_json_encode( $dec, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT ) . "\n```";
	}

	/** Pull pre-rendered HTML out of envelope (JSON-mode generators). */
	public static function extract_section_html( $json_str ) {
		$dec = json_decode( (string) $json_str, true );
		if ( ! is_array( $dec ) ) { return ''; }
		if ( ! empty( $dec['content_html'] ) && is_string( $dec['content_html'] ) ) {
			return (string) $dec['content_html'];
		}
		// On-the-fly render if we have content_json + render hint but no cached HTML
		// (e.g. older row generated before HTML caching was added).
		if ( ! empty( $dec['content_json'] ) && is_array( $dec['content_json'] ) && class_exists( 'BCPro_Section_Renderer' ) ) {
			$render = (string) ( $dec['render'] ?? '' );
			return BCPro_Section_Renderer::render( $render, $dec['content_json'] );
		}
		return '';
	}

	/**
	 * Look up a generator definition (with output/render/json_schema/system fields)
	 * from BizCoach-Pro's file-based template registry. Returns null if not found.
	 *
	 * @param string $coach_type Template slug (vd: career_coach).
	 * @param string $gen_key    Generator key (vd: gen_overview).
	 * @return array|null
	 */
	public static function find_generator_def( $coach_type, $gen_key ) {
		if ( ! class_exists( 'BizCoach_Pro_Template_Registry' ) ) { return null; }
		$tpl = BizCoach_Pro_Template_Registry::get( $coach_type );
		if ( ! is_array( $tpl ) || empty( $tpl['generators'] ) ) { return null; }
		foreach ( (array) $tpl['generators'] as $g ) {
			if ( ( $g['key'] ?? '' ) === $gen_key ) {
				return $g;
			}
		}
	...
	/**
	 * Best-effort JSON parse of an LLM response. Strips ```json``` fences,
	 * trims pre/post-amble, then json_decode. Returns array on success, null otherwise.
	 *
	 * @param string $raw
	 * @return array|null
	 */
	public static function parse_json_response( $raw ) {
		...
		// Second attempt: extract first {...} balanced block.
		$start = strpos( $raw, '{' );
		$end   = strrpos( $raw, '}' );
	...
	public static function rest_ai_fill( WP_REST_Request $req ) {
		...
		$model = self::JSON_GEN_MODEL; // Haiku: cheap, fast, follows JSON contract reliably.
