...
function bccm_render_bizcoach_map($biz){
  ...
  // =====================================================================
  // Overview (ưu tiên các field của TikTokCoach template)
  // =====================================================================
  $ov = is_array($biz['overview'] ?? null) ? $biz['overview'] : [];
  if ($ov){
    echo '<div class="bccm-card tight">';
    echo '<div class="bccm-h2">Kế hoạch 90 ngày — Overview</div>';

    echo '<div class="bccm-grid">';
      echo '<div>';
        echo '<div class="bccm-h3">Chủ đề</div>';
        echo '<p><strong>'.esc_html($ov['theme'] ?? '90-Day Strategy Canvas').'</strong></p>';

        echo '<div class="bccm-h3">Mục tiêu</div>';
        echo '<p>'.esc_html($ov['objective'] ?? 'Xây & triển khai canvas chiến lược 90 ngày để tăng trưởng.').'</p>';
      echo '</div>';

      echo '<div>';
        echo '<div class="bccm-h3">Kết quả đầu ra</div>';
        echo '<p>'.esc_html($ov['output'] ?? 'Bản đồ 90 ngày chuyển hoá + báo cáo đánh giá').'</p>';

        if (!empty($ov['company']) || !empty($ov['industry'])) {
          echo '<div class="bccm-h3">Doanh nghiệp</div><p>';
          if (!empty($ov['company']))  echo '<span class="pill">'.esc_html($ov['company']).'</span> ';
          if (!empty($ov['industry'])) echo '<span class="pill">'.esc_html($ov['industry']).'</span>';
          echo '</p>';
        }

        // Back-compat: nếu có north_star, key_kpis, assumptions
        if (!empty($ov['north_star'])) {
          echo '<div class="bccm-h3">North Star</div><p>'.esc_html($ov['north_star']).'</p>';
        }
        if (!empty($ov['key_kpis'])) {
          echo '<div class="bccm-h3">Key KPIs</div>';
          $render_list((array)$ov['key_kpis']);
        }
        if (!empty($ov['assumptions'])) {
          echo '<div class="bccm-h3">Assumptions</div>';
          $render_list((array)$ov['assumptions']);
        }
      echo '</div>';
    echo '</div>'; // grid
  ...
  // =====================================================================
  // Phases + Modules (TikTokCoach: mỗi phase có modules + weeks)
  // =====================================================================
  $phases = is_array($biz['phases'] ?? null) ? $biz['phases'] : [];
  if ($phases){
    foreach ($phases as $p){
      if (!is_array($p)) continue;
      $p_title = (string)($p['title'] ?? 'Phase');

      echo '<div class="bccm-card tight">';
      echo '<div class="bccm-h2">'.esc_html($p_title).'</div>';

      // Modules
      $modules = is_array($p['modules'] ?? null) ? $p['modules'] : [];
      if ($modules){
        foreach ($modules as $m){
          if (!is_array($m)) continue;

          $m_title    = (string)($m['title'] ?? 'Module');
          ...
          echo '<div class="bccm-card2" style="margin:14px 0 0">';
            echo '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">';
              echo '<div class="bccm-h3" style="margin:0">'.esc_html($m_title).'</div>';
              if ($day_range) echo '<span class="pill">⏱ '.esc_html($day_range).'</span>';
            echo '</div>';

            if ($objectives){
              echo '<div class="bccm-h3">Objectives</div>';
              $render_list($objectives);
            }

            echo '<div class="bccm-grid" style="margin-top:8px">';
              echo '<div>';
                if ($kpis){
                  echo '<div class="bccm-h3">KPIs</div>';
                  $render_list($kpis);
                }
                if ($tools){
                  echo '<div class="bccm-h3">Tools</div><p>';
                  $render_pills($tools);
                  echo '</p>';
                }
              echo '</div>';
              ...
              echo '<div>';
                if ($tasks){
                  echo '<div class="bccm-h3">Tasks</div>';
                  $render_list($tasks);
                }
                if ($owners){
                  echo '<div class="bccm-h3">Owners</div><p>';
                  $render_pills($owners);
                  echo '</p>';
                }
                if ($outputs){
                  echo '<div class="bccm-h3">Outputs</div>';
                  $render_list($outputs);
                }
              echo '</div>';
            echo '</div>'; // grid
      ...
      // Weeks timeline (nếu có)
      $weeks = is_array($p['weeks'] ?? null) ? $p['weeks'] : [];
      if ($weeks){
        echo '<div class="bccm-card" style="margin-top:14px">';
        echo '<div class="bccm-h3">'.esc_html($p_title).' — Weeks Timeline</div>';
        echo '<div class="bccm-grid">';
        foreach ($weeks as $w){
          ...
          echo '<div class="week-tile">';
            echo '<div class="title">'.esc_html($w_title).'</div>';
            if ($mil)   echo '<div><em>Milestones:</em> '.esc_html(implode(', ', $mil)).'</div>';
            if ($wkpis) echo '<div><em>KPIs:</em> '.esc_html(implode(', ', $wkpis)).'</div>';
            if ($wtasks)echo '<div><em>Tasks:</em> '.esc_html(implode(', ', $wtasks)).'</div>';
          echo '</div>';
        }
        echo '</div></div>';
      }
  ...
  // =====================================================================
  // Content Scripts (~30 kịch bản) — đặc thù TikTokCoach
  // =====================================================================
  $scripts = is_array($biz['content_scripts'] ?? null) ? $biz['content_scripts'] : [];
  if ($scripts){
    echo '<div class="bccm-card">';
    echo '<div class="bccm-h2">Content Scripts (~'.esc_html(count($scripts)).')</div>';
    echo '<p class="muted">Kịch bản theo phong cách cá nhân (map theo pillars, fighting topics, pain points). Mỗi kịch bản gồm hook, outline, CTA, caption, hashtags...</p>';

    // Grid render 2 cột (responsive)
    echo '<div class="bccm-grid">';
    foreach ($scripts as $i => $sc){
      if (!is_array($sc)) continue;

      $title   = (string)($sc['title'] ?? ('Script #'.($i+1)));
      $hook    = (string)($sc['hook'] ?? '');
      $outline = (array)($sc['outline'] ?? []);
      $style   = (array)($sc['style'] ?? []);
      $dur     = (int)   ($sc['duration_sec'] ?? 0);
      $cta     = (string)($sc['cta'] ?? '');
      $caption = (string)($sc['caption'] ?? '');
      $tags    = (array)($sc['hashtags'] ?? []);
      $assets  = (array)($sc['assets'] ?? []);
      $broll   = (array)($sc['broll'] ?? []);
      $reason  = (string)($sc['reason'] ?? '');

      echo '<div class="pillbox" style="padding:12px 14px">';
        echo '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">';
          echo '<div style="font-weight:700">'.esc_html($title).'</div>';
          if ($dur>0) echo '<span class="pill">~'.esc_html($dur).'s</span>';
        echo '</div>';

        if ($style){
          echo '<div class="bccm-h3" style="margin-top:8px">Style</div><p>';
          $render_pills($style);
          echo '</p>';
        }

        if ($hook){
          echo '<div class="bccm-h3">Hook</div><p>'.esc_html($hook).'</p>';
        }

        if ($outline){
          echo '<div class="bccm-h3">Outline</div>';
          $render_list($outline);
        }

        if ($cta){
          echo '<div class="bccm-h3">CTA</div><p>'.esc_html($cta).'</p>';
        }

        if ($caption){
          echo '<div class="bccm-h3">Caption</div><p>'.esc_html($caption).'</p>';
        }

        if ($tags){
          echo '<div class="bccm-h3">Hashtags</div><p>';
          $render_pills($tags);
          echo '</p>';
        }
        ...
        if ($assets || $broll){
          echo '<div class="bccm-grid" style="margin-top:8px">';
            echo '<div>';
              if ($assets){
                echo '<div class="bccm-h3">Assets</div>';
                $render_list($assets);
              }
            echo '</div>';
            echo '<div>';
              if ($broll){
                echo '<div class="bccm-h3">B-roll</div>';
                $render_list($broll);
              }
            echo '</div>';
          echo '</div>';
        ...
        if ($reason){
          echo '<p class="muted" style="margin-top:6px"><em>Lý do/ghi chú:</em> '.esc_html($reason).'</p>';
        }
      echo '</div>'; // pillbox
    }
    echo '</div>'; // grid
