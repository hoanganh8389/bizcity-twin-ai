...
function bccm_astro_enrich_aspects($aspects, $positions) {
    ...
    // Map of TitleCase aspect labels for symbol/color lookups.
    static $aspect_titlecase = [
        'conjunction' => 'Conjunction', 'opposition'   => 'Opposition',
        'trine'       => 'Trine',       'square'       => 'Square',
        'sextile'     => 'Sextile',     'quincunx'     => 'Quincunx',
        'semisextile' => 'Semisextile', 'semisquare'   => 'Semisquare',
        'sesquisquare'=> 'Sesquisquare','quintile'     => 'Quintile',
        'biquintile'  => 'Biquintile',
    ];

    // Title-case helper for planet/aspect names that may arrive lowercased
    // (e.g. "sun" → "Sun", "true_node" → "True Node").
    ...
    foreach ($aspects as $asp) {
        if (!is_array($asp)) continue;

        // Planet 1 — accept legacy `planet_1`/`planet_1_en`, V2 normalizer `p1`,
        // FAA raw `aspecting`/`aspecting_planet`, or array shape `{en: ...}`.
        $p1_raw = $asp['planet_1_en']
            ?? $asp['planet_1']
            ?? $asp['p1']
            ?? $asp['planet1']
            ?? $asp['aspecting']
            ?? $asp['aspecting_planet']
            ?? '';
        if (is_array($p1_raw)) {
            $p1_raw = $p1_raw['en'] ?? $p1_raw['name'] ?? $p1_raw['name_en'] ?? '';
        }
        $p1 = $normalize_name($p1_raw);

        // Planet 2
        $p2_raw = $asp['planet_2_en']
            ?? $asp['planet_2']
            ?? $asp['p2']
            ?? $asp['planet2']
            ?? $asp['aspected']
            ...
            ?? '';
        if (is_array($p2_raw)) {
            $p2_raw = $p2_raw['en'] ?? $p2_raw['name'] ?? $p2_raw['name_en'] ?? '';
        }
        $p2 = $normalize_name($p2_raw);

        // Aspect type — accept `aspect`, `aspect_en`, `type`, `type_en`,
        // `type_key`, or array shape.
        $type_raw = $asp['aspect_en']
            ?? $asp['aspect']
            ?? $asp['type_en']
            ?? $asp['type']
            ?? $asp['type_key']
        ...
        if (is_array($type_raw)) {
            $type_raw = $type_raw['en'] ?? $type_raw['name'] ?? '';
        }
        $type_lc = strtolower((string) $type_raw);
        $type    = $aspect_titlecase[$type_lc] ?? ucfirst($type_lc);

        // Orb — prefer caller-supplied value (V2 normalizer already computes
        // it), fall back to recomputing from positions.
        $orb = null;
        if (array_key_exists('orb', $asp) && is_numeric($asp['orb'])) {
            $orb = round((float) $asp['orb'], 4);
        } elseif (isset($asp['orbit']) && is_numeric($asp['orbit'])) {
            $orb = round((float) $asp['orbit'], 4);
        } elseif (isset($asp['orb_value']) && is_numeric($asp['orb_value'])) {
            $orb = round((float) $asp['orb_value'], 4);
        } elseif (isset($positions[$p1]['full_degree']) && isset($positions[$p2]['full_degree'])) {
            $calc = bccm_astro_calculate_orb(
                $positions[$p1]['full_degree'],
                $positions[$p2]['full_degree'],
                $type
            );
            if ($calc !== false) $orb = $calc;
        }
        ...
        $enriched[] = [
            ...
            'aspect_en'   => $type,
            'orb'         => $orb,
            'orb_dms'     => $orb !== null ? bccm_astro_decimal_to_dms($orb, true) : '—',
        ];
    }
