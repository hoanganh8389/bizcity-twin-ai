...
if ( defined( 'WP_DEBUG' ) && WP_DEBUG && ! defined( 'BCCM_ASTRO_VEDIC_DEPRECATED_NOTICE_SHOWN' ) ) {
    define( 'BCCM_ASTRO_VEDIC_DEPRECATED_NOTICE_SHOWN', true );
    if ( ! ( function_exists( 'apply_filters' ) && apply_filters( 'bcpro_silence_legacy_astro_deprecations', false ) ) ) {
        @trigger_error( '[bizcoach-pro][G.4] astro-api-vedic.php is DEPRECATED — migrate callers to BizCoach_Pro_Astro_Client::calculate_vedic()/dasha_vedic()/gochar_vedic().', E_USER_DEPRECATED );
    }
}
...
function bccm_vedic_save_chart($coachee_id, $chart_data, $birth_input = [], $passed_user_id = null) {
    ...
    // Build summary (compact UI + LLM-prompt fields).
    $summary = [
        ...
        '_source'           => isset($chart_data['_source']) ? $chart_data['_source'] : '',
    ];

    // Build traits (full enrichment payload — dasha, yogas, vargas, panchang…).
    $traits = [
        'planets'      => isset($chart_data['planets']) ? $chart_data['planets'] : [],
        'houses'       => isset($chart_data['houses']) ? $chart_data['houses'] : [],
        'navamsa'      => isset($chart_data['navamsa']) ? $chart_data['navamsa'] : [],
        'vargas'       => isset($chart_data['vargas']) ? $chart_data['vargas'] : [],
        'lagna'        => isset($chart_data['lagna']) ? $chart_data['lagna'] : [],
        'dasha'        => isset($chart_data['dasha']) ? $chart_data['dasha'] : [],
        'yogas'        => isset($chart_data['yogas']) ? $chart_data['yogas'] : [],
        'panchang'     => isset($chart_data['panchang']) ? $chart_data['panchang'] : [],
        'shadbala'     => isset($chart_data['shadbala']) ? $chart_data['shadbala'] : [],
        'ashtakavarga' => isset($chart_data['ashtakavarga']) ? $chart_data['ashtakavarga'] : [],
        'aspects'      => isset($chart_data['aspects']) ? $chart_data['aspects'] : [],
    ...
    // Lưu ý: nếu schema chưa có chart_type, không thể phân biệt vedic vs western
    // → tìm row hiện có cho coachee này; nếu đã có (Western chiếm trước) thì bỏ qua
    // lưu Vedic để không đè mất Western trên schema cũ (uniq_coachee).
    if (!$has_chart_type) {
        $legacy_existing = $wpdb->get_var($wpdb->prepare(
            "SELECT id FROM $t_astro WHERE coachee_id=%d ORDER BY id DESC LIMIT 1",
            $coachee_id
        ));
        if ($legacy_existing) {
            error_log("[BCCM Vedic] Skipped save for coachee_id={$coachee_id}: legacy schema (no chart_type column) already has a row — run migration 0.1.0.40+ to enable dual chart storage.");
            return false;
        }
    }
