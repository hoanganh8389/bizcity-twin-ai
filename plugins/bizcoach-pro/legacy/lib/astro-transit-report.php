...
function bccm_transit_report_handler() {
    ...
    // Sprint H.6 — public hash auth bypass. Accept either:
    //   chart_type='transit' (new transit-specific share URL /my-transit-astrology/)
    //   chart_type='western' (legacy — back-compat for old admin links)
    $public_ok = function_exists('bcpro_astro_public_ctx_matches')
        && ( bcpro_astro_public_ctx_matches($coachee_id, 'transit')
          || bcpro_astro_public_ctx_matches($coachee_id, 'western') );
    if (!$public_ok) {
    ...
    if (in_array($period, ['month', 'year', 'custom'], true)) {
        ...
        if (function_exists('bccm_transit_timeline_handler')) {
            bccm_transit_timeline_handler(); // exits on success; returns on API failure
        }
        // If still running here → timeline API unavailable, fall through to snapshot.
    }
    ...
    // ── Time range config ──
    $time_configs = [
        'day'   => ['period' => 'day',   'days' => 1,   'label' => 'Hôm nay (24 giờ)',      'label_short' => 'Hôm nay'],
    ...
    $is_public_view = !empty($ctx['chart_type']) && $ctx['chart_type'] === 'transit'
                      && function_exists('bcpro_get_transit_public_url');
    if ($is_public_view) {
        $nav_day   = bcpro_get_transit_public_url($coachee_id, 'day');
        $nav_week  = bcpro_get_transit_public_url($coachee_id, 'week');
        $nav_month = bcpro_get_transit_public_url($coachee_id, 'month');
        $nav_year  = bcpro_get_transit_public_url($coachee_id, 'year');
        $nav_back  = home_url('/');
    } else {
        $nav_day   = admin_url('admin-ajax.php?action=bccm_transit_report&coachee_id=' . $coachee_id . '&period=day&_wpnonce='   . $transit_nonce);
        $nav_week  = admin_url('admin-ajax.php?action=bccm_transit_report&coachee_id=' . $coachee_id . '&period=week&_wpnonce='  . $transit_nonce);
        $nav_month = admin_url('admin-ajax.php?action=bccm_transit_report&coachee_id=' . $coachee_id . '&period=month&_wpnonce=' . $transit_nonce);
        $nav_year  = admin_url('admin-ajax.php?action=bccm_transit_report&coachee_id=' . $coachee_id . '&period=year&_wpnonce='  . $transit_nonce);
        $nav_back  = admin_url('admin.php?page=bccm_my_profile');
    }
...
.toolbar .btn-day   { background: #06b6d4; }
...
/* Pro feature notice */
.pro-notice { display:flex; align-items:flex-start; gap:14px; background:linear-gradient(135deg,#fffbeb,#fef3c7); border:2px solid #fbbf24; border-radius:12px; padding:16px 20px; margin:16px 20px; }
.pro-notice .pro-icon { font-size:28px; flex-shrink:0; }
.pro-notice .pro-content { flex:1; }
.pro-notice .pro-content strong { font-size:14px; color:#92400e; display:block; margin-bottom:4px; }
.pro-notice .pro-content p { font-size:12px; color:#78350f; margin:0; line-height:1.6; }
.pro-notice .pro-content p code { background:#fde68a; padding:1px 5px; border-radius:3px; font-size:11px; }
.pro-cta { flex-shrink:0; display:inline-block; background:linear-gradient(135deg,#f59e0b,#d97706); color:#fff; font-weight:700; font-size:12px; padding:8px 16px; border-radius:8px; text-decoration:none; white-space:nowrap; align-self:center; }
.pro-cta:hover { filter:brightness(1.1); }
...
<!-- TOOLBAR -->
<div class="toolbar">
    <a href="<?php echo esc_url($nav_day); ?>" class="btn-day<?php  echo $period === 'day'   ? ' btn-active' : ''; ?>">🌅 Hôm nay</a>
    <a href="<?php echo esc_url($nav_week); ?>" class="btn-week<?php echo $period === 'week'  ? ' btn-active' : ''; ?>">📅 Tuần tới</a>
    <a href="<?php echo esc_url($nav_month); ?>" class="btn-month<?php echo in_array($period,['month','custom'],true) ? ' btn-active' : ''; ?>">🗓️ Tháng tới</a>
    <a href="<?php echo esc_url($nav_year); ?>" class="btn-year<?php echo $period === 'year'  ? ' btn-active' : ''; ?>">📆 Năm tới</a>
    <a href="javascript:window.print()" class="btn-back" style="background:#64748b">🖨️ In / PDF</a>
    <a href="<?php echo esc_url($nav_back); ?>" class="btn-back">← Quay lại</a>
    <div class="hint">Transit Report · <?php echo esc_html($time_range['label']); ?> · Dịch chuyển sao so với natal chart</div>
</div>
