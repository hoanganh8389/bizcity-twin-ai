...
function bccm_natal_report_full_handler() {
    ...
    // Sprint H.6 — accept public hash-auth as alternative to admin nonce.
    // BizCoach_Pro_Astro_Public_Router populates $GLOBALS['bcpro_public_astro_ctx']
    // before invoking this handler; if absent fall back to legacy admin gating.
    $public_ok = function_exists('bcpro_astro_public_ctx_matches')
        && bcpro_astro_public_ctx_matches($coachee_id, $chart_type);
    if (!$public_ok) {
    ...
    /* ── Sprint H.4/H.5 — Chinese and Vedic each have a fully separate renderer
     * because their stored data shapes differ from the Western schema.
     * Chinese: pillars/day_master/five_elements, no positions/houses/aspects.
     * Vedic:   planets/houses/dasha/yogas/navamsa + North Indian chart SVG. ── */
    if ($chart_type === 'chinese' && function_exists('bccm_chinese_render_natal_report')) {
        bccm_chinese_render_natal_report($astro_row, $coachee);
        exit;
    }
    if ($chart_type === 'vedic' && function_exists('bccm_vedic_render_natal_report')) {
    ...
    // Sprint H.6 — public hash auth fallback for section AJAX (no admin nonce).
...
<div class="toolbar no-print">
  ...
  <?php
  // Sprint H.6 — prefer hash-protected public URL when serving from the
  // public router so the "Tạo lại AI" button keeps working without admin login.
  if ( ! empty( $public_hash ) && class_exists( 'BizCoach_Pro_Astro_Public_Router' ) ) {
      $regen_url = BizCoach_Pro_Astro_Public_Router::get_public_url( $coachee_id, $chart_type, true );
  } else {
      $regen_url = admin_url('admin-ajax.php?action=bccm_natal_report_full&coachee_id=' . $coachee_id . '&chart_type=' . $chart_type_esc . '&regenerate=1&_wpnonce=' . wp_create_nonce('bccm_natal_report_full'));
  }
  ?>
...
(function(){
  ...
  var ahash   = <?php echo wp_json_encode($public_hash); ?>;
  var hashQS  = ahash ? ('&hash=' + encodeURIComponent(ahash)) : '';
  ...
  function loadSection(idx) {
    ...
    var url = ajaxUrl + '?action=bccm_llm_section&coachee_id=' + cid + '&chart_type=' + encodeURIComponent(ctype) + '&section=' + idx + '&_wpnonce=' + nonce + hashQS;
  ...
  window.retrySection = function(idx) {
    ...
    var url = ajaxUrl + '?action=bccm_llm_section&coachee_id=' + cid + '&chart_type=' + encodeURIComponent(ctype) + '&section=' + idx + '&regenerate=1&_wpnonce=' + nonce + hashQS;
...
// Sprint H.6 — allow public hash-auth callers (no login).
add_action('wp_ajax_nopriv_bccm_llm_section', 'bccm_llm_section_handler');
...
function bccm_llm_section_handler() {
    ...
    // Sprint H.6 — hash-auth fallback (lets shared report pages lazy-load sections
    // without a WordPress admin nonce).
    $hash_in = isset($_GET['hash']) ? sanitize_text_field( wp_unslash( $_GET['hash'] ) ) : '';
    $public_ok = false;
    if ( $hash_in !== '' && class_exists( 'BizCoach_Pro_Astro_Public_Router' ) ) {
        $public_ok = BizCoach_Pro_Astro_Public_Router::verify_hash( $coachee_id, $chart_type, $hash_in );
        if ( $public_ok ) {
    ...
    // Use Chinese / Vedic / Western context + sections depending on chart_type.
    $name = $coachee['full_name'] ?? 'Người dùng';
