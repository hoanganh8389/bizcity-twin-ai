...
function bccm_upsert_profile(array $data, $editing_id = 0){
  ...
  if ($editing_id){
    $row = bccm_filter_profile_row_to_existing_columns($row, false);
    ...
    $row['created_at'] = current_time('mysql');
    $row = bccm_filter_profile_row_to_existing_columns($row, true);
    $wpdb->insert($t['profiles'], $row);
...
/**
 * Loại bỏ các key không phải cột thật của bảng profiles, để $wpdb->insert/update
 * không fail khi subsite chưa migrate (vd: cột `platform_type` bị thiếu).
 */
if (!function_exists('bccm_filter_profile_row_to_existing_columns')) {
  function bccm_filter_profile_row_to_existing_columns(array $row, bool $is_insert = true): array {
    if (!function_exists('bccm_profile_db_columns')) return $row;
    $cols = bccm_profile_db_columns();
    if (empty($cols)) return $row;
    $cols_flip = array_flip($cols);
    $row = array_intersect_key($row, $cols_flip);

    if (!$is_insert) {
      // UPDATE path — chỉ ghi key có sẵn trong $row, không inject default
      // để tránh wipe các cột JSON đang có dữ liệu.
      return $row;
    }

    // Schema-drift safety: legacy installs có vision_json/swot_json/... LONGTEXT
    // NOT NULL không có default. INSERT mà thiếu key sẽ fail dưới strict mode.
    // Migration 0.1.0.38 đã ALTER MODIFY về NULL DEFAULT NULL nhưng có thể chưa
    // chạy trên subsite này — inject '' cho các cột JSON nếu chưa có trong $row.
    $json_cols = [
    ...
    foreach ($json_cols as $jc) {
      if (isset($cols_flip[$jc]) && !array_key_exists($jc, $row)) {
        $row[$jc] = '';
      }
    }
    return $row;
