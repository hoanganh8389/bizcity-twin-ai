...
function bccm_get_or_create_user_coachee($user_id, $platform = 'WEBCHAT', $coach_type = 'mental_coach') {
  ...
  // Auto-create from WP user data. Don't bail when get_userdata() returns false:
  // on HyperDB sharded multisites the user row may be on master while the
  // current request reads from a slave that hasn't replicated yet. We can still
  // build a usable placeholder coachee from the numeric user_id; downstream
  // code refreshes display_name/phone the next time the user is found.
  $user = get_userdata($user_id);

  $insert_data = [
    'user_id'    => $user_id,
    'coach_type' => $coach_type,
    'full_name'  => $user ? ( $user->display_name ?: $user->user_login ) : ( 'User #' . (int) $user_id ),
  ...
  // Schema-drift safety: drop unknown cols + inject '' for legacy NOT NULL JSON
  // columns so INSERT doesn't fail on subsites missing the migration.
  $insert_row = function_exists('bccm_filter_profile_row_to_existing_columns')
    ? bccm_filter_profile_row_to_existing_columns($insert_data)
    : $insert_data;

  $wpdb->insert($t['profiles'], $insert_row);

  $new_id = (int) $wpdb->insert_id;
  if ( $new_id <= 0 ) return null;

  // Avoid re-SELECTing the row we just wrote: on sharded multisites the read
  // goes to a slave that hasn't replicated yet, so the SELECT comes back NULL
  // even though the INSERT succeeded on master. Return the data we just wrote
  // (merged with the new id) so callers always get a usable row.
...
function bccm_admin_my_profile() {
  ...
  if ($has_western || $has_vedic) {
    ...
    if ($has_western) {
      ...
      $self_west = $bcpro_router_ok ? BizCoach_Pro_Astro_Public_Router::get_public_url( $coachee_id, 'western' )
      ...
      $self_west_regen = $bcpro_router_ok ? BizCoach_Pro_Astro_Public_Router::get_public_url( $coachee_id, 'western', true )
      ...
      $self_vedic = $bcpro_router_ok ? BizCoach_Pro_Astro_Public_Router::get_public_url( $coachee_id, 'vedic' )
      ...
      $self_chinese = $bcpro_router_ok ? BizCoach_Pro_Astro_Public_Router::get_public_url( $coachee_id, 'chinese' ) : '';
      echo '<a href="' . esc_url($self_west) . '" target="_blank" class="button" style="background:#3b82f6;color:#fff;border-color:#2563eb">🤖 Luận Giải AI — Western</a>';
      echo '<a href="' . esc_url($self_west_regen) . '" target="_blank" class="button" style="background:#f59e0b;color:#fff;border-color:#d97706" title="Tạo lại báo cáo Western (xóa cache)">🔄 Tạo lại Western</a>';
    }
    if ($has_vedic) {
      echo '<a href="' . esc_url($self_vedic) . '" target="_blank" class="button" style="background:#7c3aed;color:#fff;border-color:#6d28d9">🕉️ Luận Giải AI — Vedic</a>';
      if ( $self_chinese !== '' ) {
          echo '<a href="' . esc_url($self_chinese) . '" target="_blank" class="button" style="background:#dc2626;color:#fff;border-color:#b91c1c">☯️ Luận Giải AI — Chinese (Tứ Trụ)</a>';
      }
    }
    ...
    if (!empty($positions)) {
      ...
      echo '</tbody></table>';
      echo '<p class="description" style="margin-top:4px;font-size:11px;color:#3b82f6">Nguồn: Free Astro API — api.freeastroapi.com</p>';
      echo '</div></div>';
    }
