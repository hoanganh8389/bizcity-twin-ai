...
function bccm_get_natal_chart_url_by_user($user_id) {
  ...
  // Schema-aware: một số subsite cũ chưa có cột user_id → query qua coachees thay vì.
  ...
  if ($has_user_id) {
    ...
    // Fallback: join qua bảng coachees
    $t       = function_exists('bccm_tables') ? bccm_tables() : [ 'profiles' => $wpdb->prefix . 'bccm_coachees' ];
    $coachee_id = $wpdb->get_var($wpdb->prepare(
      "SELECT a.coachee_id FROM $t_astro a INNER JOIN {$t['profiles']} c ON c.id = a.coachee_id WHERE c.user_id = %d AND (a.summary IS NOT NULL OR a.traits IS NOT NULL) ORDER BY a.id DESC LIMIT 1",
      $user_id
    ));
  }
