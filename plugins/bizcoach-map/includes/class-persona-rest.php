<?php
/**
 * BizCoach — Persona REST endpoints (Wave 0.18.2)
 *
 * Powers the React "Personal Artifact Dialog" inside TwinChat. When a user
 * clicks the "🔮 Tạo bản đồ sao" smart-source chip on a notebook bound to
 * a `bizcoach` Twin Guru, the dialog calls these endpoints to:
 *
 *  1. List the user's existing coachee profiles (`bccm_coachees`) +
 *     status of their natal/transit charts (`bccm_astro`).
 *  2. Resolve share URLs (with hash + nonces) so the FE can render the
 *     same buttons as the admin "Hồ sơ thành viên" page (Xem Bản Đồ Sao,
 *     Luận Giải AI Western/Vedic, Transit Tuần/Tháng/Năm tới).
 *  3. The dialog then POSTs the chosen URL into the notebook via the
 *     existing `POST /wp-json/bizcity-twinchat/v1/sources/{nbId}` flow —
 *     no new ingest path needed.
 *
 * Namespace: bizcity-bizcoach/v1
 * Routes:
 *   GET /persona/profiles        → list coachees of current user
 *   GET /persona/links/{coachee_id} → resolve share URLs for one coachee
 *
 * @package BizCoach_Map
 * @since   0.2.0 (Wave 0.18.2)
 */

defined( 'ABSPATH' ) || exit;

if ( class_exists( 'BizCoach_Persona_Rest' ) ) return;

class BizCoach_Persona_Rest {

	const NAMESPACE = 'bizcity-bizcoach/v1';

	public static function init() {
		add_action( 'rest_api_init', [ __CLASS__, 'register_routes' ] );
	}

	public static function register_routes() {
		register_rest_route( self::NAMESPACE, '/persona/profiles', [
			'methods'             => 'GET',
			'callback'            => [ __CLASS__, 'list_profiles' ],
			'permission_callback' => [ __CLASS__, 'permission' ],
		] );
		register_rest_route( self::NAMESPACE, '/persona/links/(?P<coachee_id>\d+)', [
			'methods'             => 'GET',
			'callback'            => [ __CLASS__, 'get_links' ],
			'permission_callback' => [ __CLASS__, 'permission' ],
		] );
		// Wave 0.18.2.b — server-side fetch + ingest (avoids HTTP 400 from
		// admin-ajax cookie-protected URLs when KG ingest does wp_remote_get).
		register_rest_route( self::NAMESPACE, '/persona/ingest', [
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'ingest_link' ],
			'permission_callback' => [ __CLASS__, 'permission' ],
		] );
	}

	public static function permission() {
		return is_user_logged_in();
	}

	/**
	 * GET /persona/profiles
	 *
	 * Returns coachees owned by the current user, each enriched with:
	 *   - has_natal: boolean (chart_type=western OR vedic exists in bccm_astro)
	 *   - chart_types: ['western', 'vedic']
	 *   - links: same shape as get_links() (saves an extra round-trip)
	 *
	 * @return WP_REST_Response
	 */
	public static function list_profiles( WP_REST_Request $req ) {
		global $wpdb;

		$user_id = get_current_user_id();
		$limit   = max( 1, min( 100, (int) ( $req->get_param( 'limit' ) ?: 50 ) ) );
		$search  = trim( (string) $req->get_param( 'search' ) );

		$coachees_tbl = $wpdb->prefix . 'bccm_coachees';
		$astro_tbl    = $wpdb->prefix . 'bccm_astro';

		// `bccm_coachees` may use either user_id (modern) or created_by (legacy).
		// We bias toward user_id; admins see everyone.
		$where  = [ '1=1' ];
		$params = [];
		if ( ! current_user_can( 'manage_options' ) ) {
			$where[]  = 'user_id = %d';
			$params[] = $user_id;
		}
		if ( $search !== '' ) {
			$where[]  = '(full_name LIKE %s OR phone LIKE %s)';
			$like     = '%' . $wpdb->esc_like( $search ) . '%';
			$params[] = $like;
			$params[] = $like;
		}

		$sql = "SELECT id, full_name, phone, dob, platform_type, updated_at
		        FROM {$coachees_tbl}
		        WHERE " . implode( ' AND ', $where ) . "
		        ORDER BY updated_at DESC
		        LIMIT %d";
		$params[] = $limit;
		$rows = $wpdb->get_results( $wpdb->prepare( $sql, $params ), ARRAY_A );

		if ( empty( $rows ) ) {
			return rest_ensure_response( [ 'ok' => true, 'data' => [] ] );
		}

		// Batch fetch chart presence for all coachees in one query.
		$ids        = array_map( 'intval', wp_list_pluck( $rows, 'id' ) );
		$ids_csv    = implode( ',', $ids );
		$chart_rows = $wpdb->get_results(
			"SELECT coachee_id, chart_type FROM {$astro_tbl} WHERE coachee_id IN ({$ids_csv})",
			ARRAY_A
		);
		$chart_map = []; // coachee_id => [ 'western', 'vedic' ]
		foreach ( (array) $chart_rows as $cr ) {
			$cid = (int) $cr['coachee_id'];
			$chart_map[ $cid ][] = (string) $cr['chart_type'];
		}

		$out = [];
		foreach ( $rows as $r ) {
			$cid    = (int) $r['id'];
			$kinds  = $chart_map[ $cid ] ?? [];
			$out[]  = [
				'id'            => $cid,
				'full_name'     => (string) $r['full_name'],
				'phone'         => (string) ( $r['phone'] ?? '' ),
				'dob'           => (string) ( $r['dob'] ?? '' ),
				'platform_type' => (string) ( $r['platform_type'] ?? '' ),
				'updated_at'    => (string) ( $r['updated_at'] ?? '' ),
				'has_natal'     => ! empty( $kinds ),
				'chart_types'   => array_values( array_unique( $kinds ) ),
				// Wave 0.18.2.b — admin pages for "View detail" + "Edit" buttons
				// in the dialog header. Both open in a new tab.
				'view_url'      => admin_url( 'admin.php?page=bccm_user_profiles&action=view&coachee_id=' . $cid ),
				'edit_url'      => admin_url( 'admin.php?page=bccm_user_profiles&action=edit&coachee_id=' . $cid ),
				'links'         => self::build_links_for( $cid ),
			];
		}

		return rest_ensure_response( [ 'ok' => true, 'data' => $out ] );
	}

	/**
	 * GET /persona/links/{coachee_id}
	 */
	public static function get_links( WP_REST_Request $req ) {
		$cid = (int) $req['coachee_id'];
		if ( ! $cid ) {
			return new WP_Error( 'bad_request', 'coachee_id required', [ 'status' => 400 ] );
		}
		// Light authorization: non-admins can only read their own coachees.
		if ( ! current_user_can( 'manage_options' ) ) {
			global $wpdb;
			$owner = (int) $wpdb->get_var( $wpdb->prepare(
				"SELECT user_id FROM {$wpdb->prefix}bccm_coachees WHERE id = %d",
				$cid
			) );
			if ( $owner !== get_current_user_id() ) {
				return new WP_Error( 'forbidden', 'Not your coachee', [ 'status' => 403 ] );
			}
		}
		return rest_ensure_response( [
			'ok'   => true,
			'data' => self::build_links_for( $cid ),
		] );
	}

	/**
	 * Build the same set of share URLs the admin "Hồ sơ thành viên" page
	 * renders for a single coachee. Centralised so any future REST consumer
	 * (mobile, n8n, …) gets identical hash/nonce semantics.
	 *
	 * NOTE: nonces here are bound to the CURRENT user. They should only be
	 * pasted into a TwinChat source for the user that requested them.
	 */
	private static function build_links_for( int $coachee_id ): array {
		$natal_hash    = function_exists( 'bccm_generate_natal_chart_hash' )
			? bccm_generate_natal_chart_hash( $coachee_id )
			: substr( md5( $coachee_id . ( defined( 'AUTH_KEY' ) ? AUTH_KEY : 'bccm' ) ), 0, 16 );
		$report_nonce  = wp_create_nonce( 'bccm_natal_report_full' );
		$transit_nonce = wp_create_nonce( 'bccm_transit_report' );

		return [
			'natal_chart_view' => [
				'label' => '🌟 Natal Chart View',
				'url'   => home_url( '/my-natal-chart/?id=' . $coachee_id . '&hash=' . $natal_hash ),
				'kind'  => 'astro_natal_chart',
			],
			'natal_report_western' => [
				'label' => '🤖 AI Reading — Western',
				'url'   => admin_url( 'admin-ajax.php?action=bccm_natal_report_full&coachee_id=' . $coachee_id . '&chart_type=western&_wpnonce=' . $report_nonce ),
				'kind'  => 'astro_natal_chart',
			],
			'natal_report_vedic' => [
				'label' => '🕉️ AI Reading — Vedic',
				'url'   => admin_url( 'admin-ajax.php?action=bccm_natal_report_full&coachee_id=' . $coachee_id . '&chart_type=vedic&_wpnonce=' . $report_nonce ),
				'kind'  => 'astro_natal_chart',
			],
			'transit_week' => [
				'label' => '🔮 Transit — Next Week',
				'url'   => admin_url( 'admin-ajax.php?action=bccm_transit_report&coachee_id=' . $coachee_id . '&period=week&_wpnonce=' . $transit_nonce ),
				'kind'  => 'astro_transit_report',
			],
			'transit_month' => [
				'label' => '🔮 Transit — Next Month',
				'url'   => admin_url( 'admin-ajax.php?action=bccm_transit_report&coachee_id=' . $coachee_id . '&period=month&_wpnonce=' . $transit_nonce ),
				'kind'  => 'astro_transit_report',
			],
			'transit_year' => [
				'label' => '🔮 Transit — Next Year',
				'url'   => admin_url( 'admin-ajax.php?action=bccm_transit_report&coachee_id=' . $coachee_id . '&period=year&_wpnonce=' . $transit_nonce ),
				'kind'  => 'astro_transit_report',
			],
		];
	}

	/**
	 * POST /persona/ingest
	 *
	 * Body: { notebook_id, coachee_id, link_key }
	 *
	 * Server-side fetches the resolved URL using the current user's session
	 * cookies (admin-ajax bccm_* handlers require that), strips HTML to plain
	 * text, then ingests through the standard scoped KG service so embedding
	 * + graph spawn fire normally. Returns the ingest result.
	 */
	public static function ingest_link( WP_REST_Request $req ) {
		$notebook_id = (int) $req->get_param( 'notebook_id' );
		$coachee_id  = (int) $req->get_param( 'coachee_id' );
		$link_key    = sanitize_key( (string) $req->get_param( 'link_key' ) );

		if ( ! $notebook_id || ! $coachee_id || ! $link_key ) {
			return new WP_Error( 'bad_request', 'notebook_id, coachee_id, link_key required', [ 'status' => 400 ] );
		}

		// Authorization — non-admins must own the coachee.
		if ( ! current_user_can( 'manage_options' ) ) {
			global $wpdb;
			$owner = (int) $wpdb->get_var( $wpdb->prepare(
				"SELECT user_id FROM {$wpdb->prefix}bccm_coachees WHERE id = %d",
				$coachee_id
			) );
			if ( $owner !== get_current_user_id() ) {
				return new WP_Error( 'forbidden', 'Not your coachee', [ 'status' => 403 ] );
			}
		}

		$links = self::build_links_for( $coachee_id );
		if ( ! isset( $links[ $link_key ] ) ) {
			return new WP_Error( 'invalid_link', 'Unknown link key: ' . $link_key, [ 'status' => 400 ] );
		}
		$link = $links[ $link_key ];

		// Resolve coachee name for a friendly title.
		global $wpdb;
		$coachee_name = (string) $wpdb->get_var( $wpdb->prepare(
			"SELECT full_name FROM {$wpdb->prefix}bccm_coachees WHERE id = %d",
			$coachee_id
		) );
		if ( $coachee_name === '' ) $coachee_name = 'Coachee #' . $coachee_id;

		// ── URL dedup (pre-fetch) ──────────────────────────────────────────────
		// Admin-ajax persona URLs contain a `_wpnonce` that rotates every session.
		// Store and compare using the canonical URL (nonce stripped) so that
		// clicking "+ Source" a second time — even after a page refresh — returns
		// the existing source rather than re-fetching and creating a duplicate row.
		//
		// Query bizcity_kg_sources (the unified canonical table) not the legacy
		// webchat_sources. origin_url = nonce-free URL; origin_id = webchat id.
		$canonical_url = remove_query_arg( '_wpnonce', $link['url'] );
		$kg_tbl        = class_exists( 'BizCity_KG_Database' )
			? BizCity_KG_Database::instance()->tbl_sources()
			: $wpdb->prefix . 'bizcity_kg_sources';
		$existing_origin_id = (int) $wpdb->get_var( $wpdb->prepare(
			"SELECT origin_id FROM {$kg_tbl}
			  WHERE scope_type    = 'notebook'
			    AND scope_id      = %s
			    AND origin_plugin = 'twinchat'
			    AND origin_url    = %s
			  LIMIT 1",
			(string) $notebook_id,
			$canonical_url
		) );
		if ( $existing_origin_id > 0 ) {
			return rest_ensure_response( [
				'ok'        => true,
				'duplicate' => true,
				'data'      => [ 'source_id' => $existing_origin_id ],
			] );
		}
		// ── end URL dedup ─────────────────────────────────────────────────────

		// Fetch with current session cookies so admin-ajax accepts the nonce.
		// Pass the cookies as WP_Http_Cookie objects (the array from $_COOKIE
		// is also accepted but objects are safer across WP versions).
		$cookies = [];
		foreach ( $_COOKIE as $name => $value ) {
			$cookies[] = new WP_Http_Cookie( [
				'name'  => $name,
				'value' => $value,
			] );
		}

		// Wave 0.18.5b — Markdown-First fast path (R-SMF-3).
		// Try ?_format=md + Accept: text/markdown first. If the provider supports
		// markdown-native rendering it returns Content-Type: text/markdown and we
		// skip the HTML sanitizer entirely (cheaper + zero info loss).
		$md_url = add_query_arg( [ '_format' => 'md' ], $link['url'] );
		$resp   = wp_remote_get( $md_url, [
			'timeout'     => 30,
			'redirection' => 5,
			'cookies'     => $cookies,
			'sslverify'   => false,
			'headers'     => [
				'User-Agent' => 'BizCity-Twin/1.0 (persona ingest)',
				'Accept'     => 'text/markdown,text/plain;q=0.9,text/html;q=0.5',
			],
		] );

		$markdown_native = false;
		if ( ! is_wp_error( $resp ) ) {
			$ct = (string) wp_remote_retrieve_header( $resp, 'content-type' );
			if ( stripos( $ct, 'text/markdown' ) !== false ) {
				$markdown_native = true;
			}
		}

		// Fallback to HTML fetch if markdown fast path missed/errored.
		if ( ! $markdown_native ) {
			$resp = wp_remote_get( $link['url'], [
				'timeout'     => 30,
				'redirection' => 5,
				'cookies'     => $cookies,
				'sslverify'   => false,
				'headers'     => [
					'User-Agent' => 'BizCity-Twin/1.0 (persona ingest)',
				],
			] );
		}

		if ( is_wp_error( $resp ) ) {
			return new WP_Error(
				'fetch_failed',
				'Cannot reach link: ' . $resp->get_error_message(),
				[ 'status' => 502 ]
			);
		}
		$code = (int) wp_remote_retrieve_response_code( $resp );
		if ( $code < 200 || $code >= 400 ) {
			return new WP_Error(
				'fetch_http_' . $code,
				'Source returned HTTP ' . $code,
				[ 'status' => 502 ]
			);
		}

		$body = (string) wp_remote_retrieve_body( $resp );
		$text = '';

		if ( $markdown_native ) {
			// Provider already gave us clean markdown — trust it (still normalize
			// line endings + collapse blank lines via sanitizer fast path).
			$text = $body;
		} else {
			// Try to decode JSON envelope (admin-ajax wp_send_json) first — many
			// persona handlers wrap their HTML/text inside JSON. Walk the tree to
			// pull strings >30 chars then concat into one HTML blob.
			$decoded = json_decode( $body, true );
			if ( is_array( $decoded ) ) {
				$candidates = [];
				$walk = function ( $node ) use ( &$walk, &$candidates ) {
					if ( is_string( $node ) ) {
						if ( strlen( $node ) > 30 ) $candidates[] = $node;
						return;
					}
					if ( is_array( $node ) ) {
						foreach ( $node as $v ) $walk( $v );
					}
				};
				$walk( $decoded );
				if ( ! empty( $candidates ) ) {
					$text = implode( "\n\n", array_unique( $candidates ) );
				}
			}
			if ( $text === '' ) $text = $body;
		}

		// HTML → Markdown sanitizer (R-SMF-2). Strips Query Monitor + WP chrome
		// + scripts/styles, then walks remaining DOM into clean markdown. Even
		// for markdown-native responses we run it: it's a no-op fast path when
		// no HTML tags are present and gives us consistent whitespace.
		if ( class_exists( 'BizCity_Source_HTML_Sanitizer' ) ) {
			$sanitized = BizCity_Source_HTML_Sanitizer::to_markdown( $text, [
				'min_length'   => 40,
				'strip_qm'     => true,
				'strip_chrome' => true,
				'keep_links'   => true,
				'keep_images'  => false,
			] );
			if ( is_wp_error( $sanitized ) ) {
				return new WP_Error(
					'empty_content',
					'Source returned empty/short content after sanitize: ' . $sanitized->get_error_message() . '. Try the View Detail page first to make sure the report has been generated.',
					[ 'status' => 422 ]
				);
			}
			$text = $sanitized;
		} else {
			// Hard fallback if sanitizer didn't load — keep legacy behavior.
			$text = wp_strip_all_tags( $text );
			$text = preg_replace( '/[ \t]+/', ' ', (string) $text );
			$text = preg_replace( "/\n{3,}/", "\n\n", (string) $text );
			$text = trim( (string) $text );
			if ( $text === '' || strlen( $text ) < 40 ) {
				return new WP_Error(
					'empty_content',
					'Source returned empty/short content (' . strlen( $text ) . ' chars).',
					[ 'status' => 422 ]
				);
			}
		}

		if ( ! class_exists( 'BizCity_KG' ) ) {
			return new WP_Error( 'no_kg', 'Knowledge Graph service unavailable', [ 'status' => 500 ] );
		}

		$title = $link['label'] . ' — ' . $coachee_name;
		$res = BizCity_KG::ingest(
			[ 'plugin' => 'twinchat', 'scope_id' => $notebook_id ],
			[
				'type'     => 'text',
				'title'    => $title,
				'content'  => $text,
				'url'      => $canonical_url,  // store nonce-free URL for stable dedup
				'metadata' => [
					'provider'        => 'bizcoach',
					'coachee_id'      => $coachee_id,
					'link_key'        => $link_key,
					'source_kind'     => $link['kind'],
					'format'          => 'markdown',                                  // R-SMF-1
					'sanitized_at'    => time(),                                      // R-SMF-6 audit
					'markdown_native' => $markdown_native,                            // R-SMF-3 telemetry
				],
			]
		);
		if ( is_wp_error( $res ) ) return $res;

		return rest_ensure_response( [ 'ok' => true, 'data' => $res ] );
	}
}

BizCoach_Persona_Rest::init();
