...
class BizCity_Personal_Installer {
	...
	public static function install() {
		...
		// [2026-08-20 Johnny Chu] PHASE-PROFILE-QR — Profile card registry; vCard data remains in the Page Builder site_config.
		dbDelta( "CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}bizcity_personal_profile_cards` (
			id             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			owner_user_id  BIGINT UNSIGNED NOT NULL,
			bzpb_project_id BIGINT UNSIGNED NOT NULL,
			label          VARCHAR(120) NOT NULL DEFAULT '',
			template_key   VARCHAR(60) NOT NULL DEFAULT '',
			qr_id          BIGINT UNSIGNED NULL DEFAULT NULL,
			status         ENUM('draft','published','archived') NOT NULL DEFAULT 'draft',
			created_at     DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
			updated_at     DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
			PRIMARY KEY    (id),
			UNIQUE KEY uniq_project (bzpb_project_id),
			KEY idx_owner   (owner_user_id),
			KEY idx_status  (status)
		) $charset_collate;" );

		// [2026-08-20 Johnny Chu] PHASE-PROFILE-QR — One primary QR style per Profile card; rendering stays client-side.
		dbDelta( "CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}bizcity_personal_profile_qrcodes` (
			id             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			card_id        BIGINT UNSIGNED NOT NULL,
			target_type    ENUM('profile_page','vcard_file','custom_url') NOT NULL DEFAULT 'profile_page',
			...
			updated_at     DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
			PRIMARY KEY    (id),
			UNIQUE KEY uniq_card (card_id)
		) $charset_collate;" );

		// [2026-08-20 Johnny Chu] PHASE-PROFILE-QR — Privacy-minimal public Profile events; raw IP is never stored.
		dbDelta( "CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}bizcity_personal_profile_analytics_events` (
			id            BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			card_id       BIGINT UNSIGNED NOT NULL,
			event_type    VARCHAR(40) NOT NULL DEFAULT '',
			visitor_hash  CHAR(64) NOT NULL DEFAULT '',
			referrer      VARCHAR(255) NOT NULL DEFAULT '',
			meta_json     VARCHAR(255) NULL DEFAULT NULL,
			occurred_at   DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
			PRIMARY KEY   (id),
			KEY idx_card_time (card_id, occurred_at),
			KEY idx_card_type (card_id, event_type)
		) $charset_collate;" );
...
if ( class_exists( 'BizCity_Schema_Registry' ) ) {
	...
	BizCity_Schema_Registry::register(
		'bizcity_personal_profile_cards',
		'modules.personal',
		BizCity_Personal_Installer::SCHEMA_VERSION,
		BizCity_Personal_Installer::VERSION_OPTION,
		array( 'BizCity_Personal_Installer', 'install' )
	...
	BizCity_Schema_Registry::register(
		'bizcity_personal_profile_qrcodes',
		'modules.personal',
		BizCity_Personal_Installer::SCHEMA_VERSION,
		BizCity_Personal_Installer::VERSION_OPTION,
		array( 'BizCity_Personal_Installer', 'install' )
	...
	BizCity_Schema_Registry::register(
		'bizcity_personal_profile_analytics_events',
		'modules.personal',
		BizCity_Personal_Installer::SCHEMA_VERSION,
		BizCity_Personal_Installer::VERSION_OPTION,
		array( 'BizCity_Personal_Installer', 'install' )
