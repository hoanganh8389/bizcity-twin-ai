<?php
/**
 * Diagnostics probe: twinbrain.web.tax (TBR.W17 Wave 1).
 *
...
if ( class_exists( 'BizCity_Probe_TwinBrain_Web_Tax', false ) ) {
	return;
}

final class BizCity_Probe_TwinBrain_Web_Tax implements BizCity_Diagnostics_Probe {

	const PROBE_QUERY = 'thuế TNCN biểu thuế lũy tiến 2024';

	public function id(): string          { return 'twinbrain.web.tax'; }
	public function label(): string       { return 'Web Tax — real Tavily + LLM call'; }
	public function description(): string { return 'Gọi thật BizCity_TwinBrain_Web_Tax. Kiểm tra allowlist (tct.gov/gdt.gov/mof/customs/thuvienphapluat), citation [tax:N], disclaimer 💰.'; }
	public function severity(): string    { return 'warning'; }
	public function order(): int          { return 41; }
	public function icon(): string        { return 'receipt'; }
	public function estimate_ms(): int    { return 8000; }

	public function precondition() {
		if ( ! class_exists( 'BizCity_TwinBrain_Web_Tax' ) )     return 'BizCity_TwinBrain_Web_Tax chưa load.';
	...
	public function run( $ctx ): array {
		...
		$trace_id = 'probe-web-tax-' . wp_generate_uuid4();
		$started  = microtime( true );
		try { $row = BizCity_TwinBrain_Web_Tax::instance()->run( $trace_id, self::PROBE_QUERY ); }
		...
		$native = 0;
		foreach ( (array) ( $row['citations'] ?? [] ) as $c ) if ( strpos( (string) ( $c['token'] ?? '' ), '[tax:' ) === 0 ) $native++;
		$ctx->emit_step( [ 'label' => 'Citation [tax:N#URL]', 'status' => $cite_n > 0 ? 'pass' : 'fail', 'detail' => sprintf( '%d total · %d native', $cite_n, $native ) ] );
		...
		if ( ! $ok ) {
			...
			if ( $cite_n === 0 )  $reasons[] = 'no [tax:N]';
			if ( ! $ans_ok )      $reasons[] = $is_stub ? 'stub' : 'empty';
			if ( ! $has_disc )    $reasons[] = 'no 💰';
			if ( ! $stance_ok )   $reasons[] = "stance=$stance";
			if ( $err !== '' )    $reasons[] = 'flag: ' . $err;
			return [ 'status' => 'fail', 'summary' => 'Web Tax incomplete — ' . implode( '; ', $reasons ), 'error' => implode( '; ', $reasons ), 'fix_hint' => 'Check seeder + ensure_disclaimer().' ];
		}
		return [ 'status' => 'pass', 'summary' => sprintf( 'Web Tax OK — %d/%d allowlist · %d cite · stance=%s · %dms', $tier_ok, count( $results ), $cite_n, $stance, $elapsed_ms ) ];
	}
...
add_filter( 'bizcity_diagnostics_register_probes', function ( $list ) {
	$list[] = 'BizCity_Probe_TwinBrain_Web_Tax';
	return $list;
} );
