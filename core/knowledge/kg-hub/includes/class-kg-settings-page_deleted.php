 ...
 *
 * Canonical owner: REST `bizcity-knowledge/v2/cost/settings` (BizCity_KG_Rest_Controller) + the
 * "Settings & Cost" view of modules/twinkg (`/twin/?plugin=twinkg&view=settings`).
 *
 * The PHP form moved to REST + a twinkg view (CORE-REDUCTION-WP-08 H-09 / WP-09 T4, rule R-KG-UI).
 * The old slug `bizcity-kg-hub-settings` is redirected by BizCity_TwinKG_Admin_Menu.
 *
 * Retired 2026-09-24 under R-ORPHAN-FILE: this file declares
	...
	class BizCity_KG_Settings_Page {
		...
		public function register() {
			...
			$capability = class_exists( 'BizCity_Network_Admin_Capability' )
				...
				: 'manage_options';
			// [2026-09-24 Claude Opus 5] CORE-REDUCTION-WP-09 T3 — the parent used to be
			// `BizCity_KG_Admin_Menu::PAGE_SLUG`, a class that no longer exists (the KG React mount
			// moved to modules/twinkg). Referencing its constant would fatal on every admin request.
			// This page is deliberately re-parented to the Workspace root rather than to the new
			// module page: core must not depend on a module, and a module page registered later in
			// the admin_menu order would leave this child an orphan. The page itself is scheduled to
			// become a twinkg view backed by REST (CORE-REDUCTION-WP-08 H-09 / WP-09 T4).
			add_submenu_page(
