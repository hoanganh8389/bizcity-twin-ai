...
class BizCity_KG_Skeleton_Diagnostic {
	...
	public function __construct() {
		// Admin submenu removed (Consolidation M1, 2026-06-02). Smoke moved
		// to BizCity_Probe_KG_Skeleton (core/diagnostics). Class kept for
		// CLI dispatcher (`wp bizcity diag skeleton-*`).
	}
	...
	public function roadmap( int $blog_id = 0 ): array {
		...
		try {
			// Plugin root. [2026-09-24 Claude Opus 5.5] CORE-REDUCTION WP-11 A — this file is now
			// core/kg-hub/skeleton/, so the root is 3 levels up (it was 4 under the old core/knowledge/ parent).
			$plugin_dir = defined( 'BIZCITY_TWIN_AI_DIR' ) ? untrailingslashit( BIZCITY_TWIN_AI_DIR ) : dirname( __DIR__, 3 );
			...
			$shared_dir = $plugin_dir . '/core/kg-hub/assets/';
			$shared_rel = 'core/kg-hub/assets/';
