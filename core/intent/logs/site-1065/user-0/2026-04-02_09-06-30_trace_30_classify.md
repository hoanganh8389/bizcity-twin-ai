...
| Field | Value |
|-------|-------|
| Timestamp | 2026-04-02 09:06:36 UTC |
| Trace ID | trace_30d3acc019acc3f0 |
| Site ID | 1065 |
| User ID | 0 |
...
```
## GOALS & TOOLS (5 mục — hiển thị top 5/148, ★=likely)
1. bizcoach_consult: Tư vấn chiêm tinh — User hỏi liên quan đến chiêm tinh, tử vi, vận mệnh, bản đồ sao, phong thủy, taro | cần: prompt(text) | tùy chọn: prompt_images(image)
2. warehouse_receipt: Nhập kho — Tạo phiếu nhập kho hàng hóa (tên SP, số lượng, giá mua) | cần: topic(text)
3. tarot_interpret: Giải bài Tarot — User muốn giải nghĩa lá bài Tarot — gửi ảnh hoặc nhắn tên lá bài. Đây là tool CH | cần: card_info(text) | tùy chọn: card_images(image), question_focus(text)
4. send_link_tarot: send_link_tarot — Tool phụ — Gửi/tạo link bốc bài Tarot online để user bốc bài khi không có bài tr | cần: question_focus(text), spread(number), user_id(number), session_id(text), platform(text)
5. knowledge_train: knowledge_train — Đào tạo kiến thức từ file, URL hoặc text vào Knowledge Fabric | cần: source_type(text), scope(text), url(text), content(text), source_name(text)
```

## 5. Full LLM Prompt

<details>
<summary>Click to expand full prompt (7311 chars)</summary>
...
```json
```json
{"mode":"knowledge","confidence":0.8,"is_memory":false,"memory_type":"","built_in_function":"","knowledge_type":"advisor","suggested_tool":"image_create","intent":"","goal":"","goal_label":"","entities":{},"filled_slots":[],"missing_slots":[]}
```
```
...
```json
{
    ...
    "meta": {
        "llm_tokens": {
            ...
            "total_tokens": 2523,
            "cost": 0.000908700000000000024207025273170756918261758983135223388671875,
            "is_byok": false,
            ...
            "cost_details": {
                "upstream_inference_cost": 0.000908700000000000024207025273170756918261758983135223388671875,
                "upstream_inference_prompt_cost": 0.00073620000000000000543731726310170415672473609447479248046875,
