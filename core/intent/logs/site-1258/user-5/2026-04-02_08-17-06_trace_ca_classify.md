...
| Field | Value |
|-------|-------|
| Timestamp | 2026-04-02 08:17:08 UTC |
| Trace ID | trace_ca9fdd3f881d17e2 |
| Site ID | 1258 |
| User ID | 5 |
...
```
Tạo video lipsync từ nhân vật AI
```
...
```json
{
    "regex": "\/tạo video|làm video|quay video|video sản phẩm|video quảng cáo|tao video|video clip\/ui",
    "goal": "create_video",
    "label": "Tạo video",
    "extract": [
        "title",
...
```
## GOALS & TOOLS (10 mục — hiển thị top 10/166, ★=likely)
★ 1. create_video: Tạo video — Tạo video sản phẩm, video quảng cáo, video clip từ ảnh/mô tả | cần: message(text) | tùy chọn: image_url(image), duration(choice), aspect_ratio(choice), voiceover_text(text), model(choice)
★ 2. create_video: Tạo video — Tạo video sản phẩm, video quảng cáo, video clip từ ảnh/mô tả | tùy chọn: image_url(image), message(text), duration(choice:5,10,15,20,30), aspect_ratio(choice:9:16,16:9,1:1), voiceover_text(text), model(choice:2.6|pro,2.6|std,2.5|pro,1.6|pro,seedance:1.0)
...
<details>
<summary>Click to expand full prompt (8496 chars)</summary>
...
⚡ REGEX PRE-MATCH: Tin nhắn khớp pattern của "create_video" (Tạo video). Ưu tiên goal này nếu phù hợp (★ trong danh sách).

Tin nhắn: "Tạo video lipsync từ nhân vật AI"
...
```json
{
  ...
  "intent": "new_goal",
  "goal": "create_video",
  "goal_label": "Tạo video",
  "entities": {
    "message": "lipsync từ nhân vật AI"
  },
  "filled_slots": [
    "message"
  ],
...
```json
{
    ...
    "meta": {
        "llm_tokens": {
            ...
            "total_tokens": 3036,
            "cost": 0.0012363999999999999122868299394895075238309800624847412109375,
            "is_byok": false,
            ...
            "cost_details": {
                "upstream_inference_cost": 0.0012363999999999999122868299394895075238309800624847412109375,
                "upstream_inference_prompt_cost": 0.00086640000000000002601530102452898063347674906253814697265625,
                "upstream_inference_completions_cost": 0.000369999999999999994691746163510970291099511086940765380859375
            },
