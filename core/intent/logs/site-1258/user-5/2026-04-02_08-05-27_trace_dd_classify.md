...
| Field | Value |
|-------|-------|
| Timestamp | 2026-04-02 08:05:31 UTC |
| Trace ID | trace_dda6dd020b19a975 |
| Site ID | 1258 |
| User ID | 5 |
...
```
đặt lịch chiều mai tao họp nhé
```
...
```json
{
    "regex": "\/tạo lịch|tao lich|tạo sự kiện|tao su kien|thêm lịch|them lich|hẹn lịch|hen lich|đặt lịch|dat lich|book meeting|create event|add event\/ui",
...
★ 1. scheduler_create_event: Tao su kien lich — Tao su kien lich noi bo va dong bo Google neu da ket noi. | cần: title(text), start_at(text) | tùy chọn: end_at(text), description(text), all_day(choice), reminder_min(number), source(text), ai_context(text)
...
<details>
<summary>Click to expand full prompt (8443 chars)</summary>
...
⚡ REGEX PRE-MATCH: Tin nhắn khớp pattern của "scheduler_create_event" (Tao su kien lich). Ưu tiên goal này nếu phù hợp (★ trong danh sách).

Tin nhắn: "đặt lịch chiều mai tao họp nhé"
...
```json
```json
{"mode":"execution","confidence":0.9,"is_memory":false,"memory_type":"","built_in_function":"","knowledge_type":"","suggested_tool":"","intent":"new_goal","goal":"scheduler_create_event","goal_label":"Tao su kien lich","entities":{"title":"họp","start_at":"chiều mai"},"filled_slots":["title","start_at"],"missing_slots":[]}
```
```
...
```json
{
    ...
    "meta": {
        "llm_tokens": {
            ...
            "total_tokens": 2937,
            "cost": 0.00109449999999999998678834600696063716895878314971923828125,
            "is_byok": false,
            ...
            "cost_details": {
                "upstream_inference_cost": 0.00109449999999999998678834600696063716895878314971923828125,
                "upstream_inference_prompt_cost": 0.000852000000000000001255939796607208336354233324527740478515625,
                "upstream_inference_completions_cost": 0.00024250000000000001263746052249103968279086984694004058837890625
            },
