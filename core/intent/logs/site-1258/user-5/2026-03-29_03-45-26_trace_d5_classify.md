...
| Field | Value |
|-------|-------|
| Timestamp | 2026-03-29 03:45:29 UTC |
| Trace ID | trace_d5287c070d8a8f06 |
| Site ID | 1258 |
| User ID | 5 |
...
```
viết bài marketing về xu hướng AI rồi tạo ảnh minh họa rồi đăng Facebook
```
...
```json
{
    "regex": "\/(?:viết|tạo|gen|soạn|làm)\\s*.{5,60}?\\s*(?:rồi|sau\\s*đó|tiếp\\s*theo|xong\\s*thì|rồi\\s*thì)\\s*(?:đăng|post|gửi|tạo|viết|gen|upload|publish)\/ui",
    "goal": "build_workflow",
...
```
## GOALS & TOOLS (10 mục — hiển thị top 10/136, ★=likely)
★ 1. build_workflow: Tạo kịch bản workflow — Tạo kịch bản workflow tự động hóa từ mô tả | cần: description(text) | tùy chọn: trigger_type(choice), trigger_filter(text)
2. ask_chatgpt: Hỏi ChatGPT — Dùng ChatGPT (OpenAI) để hỏi, tìm hiểu, nghiên cứu, học tập, tổng hợp, phân tích | cần: question(text)
3. translate_and_publish: Dịch và đăng bài — Dịch một bài viết ĐÃ CÓ sang ngôn ngữ khác (en, ja, ko, zh, th) và đăng bản dịch | cần: post_id(text), target_lang(choice:0,1,2,3,4) | tùy chọn: tone(choice:0,1,2)
4. contacts_list: Xem danh bạ Google — Xem danh sách liên hệ trong Google Contacts | tùy chọn: max_results(number), query(text)
...
7. post_facebook: Đăng bài Facebook — Đăng bài lên trang Facebook (nội dung, ảnh, link) | cần: message(text), image_url(url), content(text), title(text), url(url)
8. list_facebook_posts: Xem danh sách bài FB đã đăng — Liệt kê các bài Facebook AI đã tạo gần đây | tùy chọn: limit(choice:5,10,20)
9. create_facebook_post: Tạo & đăng bài Facebook — AI tạo nội dung hấp dẫn từ chủ đề, kèm ảnh, đăng lên một hoặc nhiều Facebook Pag | cần: topic(text) | tùy chọn: image_url(image), tone(choice:engaging,professional,friendly,promotional,storytelling), page_id(text)
10. write_seo_article: Viết bài chuẩn SEO — Viết bài blog/content MỚI chuẩn SEO từ chủ đề hoặc từ khóa. Bao gồm outline, nội | cần: topic(text) | tùy chọn: focus_keyword(text), tone(choice:0,1,2), length(choice:0,1), image_url(image)
```

## 5. Full LLM Prompt

<details>
<summary>Click to expand full prompt (8415 chars)</summary>
...
⚡ REGEX PRE-MATCH: Tin nhắn khớp pattern của "build_workflow" (Tạo workflow chuỗi tự động). Ưu tiên goal này nếu phù hợp (★ trong danh sách).

Tin nhắn: "viết bài marketing về xu hướng AI rồi tạo ảnh minh họa rồi đăng Facebook"
...
```json
{
  ...
  "intent": "new_goal",
  "goal": "build_workflow",
  "goal_label": "Tạo kịch bản workflow",
  "entities": {
    "description": "viết bài marketing về xu hướng AI rồi tạo ảnh minh họa rồi đăng Facebook"
  },
  "filled_slots": [
    "description"
  ],
...
```json
{
    ...
    "meta": {
        "llm_tokens": {
            ...
            "total_tokens": 2957,
            "cost": 0.00124350000000000003065603326746213497244752943515777587890625,
            "is_byok": false,
            ...
            "cost_details": {
                "upstream_inference_cost": 0.00124350000000000003065603326746213497244752943515777587890625,
                "upstream_inference_prompt_cost": 0.00083850000000000005258293800380897664581425487995147705078125,
                "upstream_inference_completions_cost": 0.000404999999999999978073095263653158326633274555206298828125
            },
