...
| Field | Value |
|-------|-------|
| Timestamp | 2026-03-31 06:03:53 UTC |
| Trace ID | trace_495078665b9a25f2 |
| Site ID | 1258 |
| User ID | 5 |
...
```
xem ngày mai công việc tôi thế nào nhé,
```
...
★ 1. bizcoach_consult: Tư vấn chiêm tinh — User hỏi bất kỳ câu nào liên quan đến chiêm tinh, tử vi, vận mệnh, bản đồ sao, d | cần: user_id(number), prompt(text) | tùy chọn: prompt_images(image)
...
3. rewrite_article: Viết lại bài viết — Viết lại / chỉnh sửa / biên tập nội dung một bài viết WordPress đã có | cần: post_id(text) | tùy chọn: instruction(text), tone(choice:0,1,2)
4. calendar_list_events: Xem lịch Google Calendar — Xem danh sách sự kiện trong Google Calendar | tùy chọn: time_min(text), time_max(text), max_results(number)
5. calendar_create_event: Tạo sự kiện Calendar — Tạo sự kiện mới trong Google Calendar | cần: title(text), start_time(text), end_time(text) | tùy chọn: description(text), attendees(text)
6. post_facebook: Đăng bài Facebook — Đăng bài lên trang Facebook (nội dung, ảnh, link) | cần: message(text), image_url(url), content(text), title(text), url(url)
...
<details>
<summary>Click to expand full prompt (8417 chars)</summary>
...
Tin nhắn: "xem ngày mai công việc tôi thế nào nhé,"
...
```json
{
  ...
  "entities": {
    "prompt": "ngày mai công việc tôi thế nào"
  },
  "filled_slots": [
    "prompt"
  ],
...
```json
{
    ...
    "meta": {
        "llm_tokens": {
            ...
            "total_tokens": 2961,
            "cost": 0.00122489999999999990380750158891487444634549319744110107421875,
            "is_byok": false,
            ...
            "cost_details": {
                "upstream_inference_cost": 0.00122489999999999990380750158891487444634549319744110107421875,
                "upstream_inference_prompt_cost": 0.000842399999999999984749698977992693471605889499187469482421875,
                "upstream_inference_completions_cost": 0.0003824999999999999732679112351974026751122437417507171630859375
            },
