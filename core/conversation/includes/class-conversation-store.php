<?php
/**
 * Bizcity Twin AI — Conversation store (shared message store)
 *
 * [2026-09-24 Claude Opus 5.5] CORE-REDUCTION WP-11 C0a / R-INTENT-MIN R-IM-6 — moved here from
 * modules/webchat/includes/class-webchat-database.php. The `bizcity_webchat_*` tables are the shared
 * message store of TwinChat, Twin GPT (/gpt/), Channel Gateway, CRM and profile, so their owner is a
 * neutral core folder, not a channel module. Table names and the class name `BizCity_WebChat_Database`
 * are kept on purpose (dozens of callers use the class name, its constants and `method_exists`);
 * a rename with an alias is a later, separate step. The old file is now a shim that loads this one.
 *
...
class BizCity_WebChat_Database {
    ...
    /** Option that records the installed schema version (unchanged from the webchat era). */
    const VERSION_OPTION = 'bizcity_webchat_db_version';

    /**
     * Create / upgrade the shared message-store tables when the recorded schema version is missing or stale.
     *
     * [2026-09-24 Claude Opus 5.5] CORE-REDUCTION WP-11 C0a — this is the callback the diagnostics installer
     * registry has always named (`[ 'BizCity_WebChat_Database', 'ensure_tables_exist' ]`), but the method
     * only existed on the webchat bootstrap class, so `is_callable()` failed and the provisioner skipped the
     * store on new blogs. The logic is the one the webchat bootstrap ran on `init`; that bootstrap now keeps
     * only its request-context check and delegates here. Idempotent: returns early when the version matches.
     *
     * @return bool True when a create/upgrade ran.
     */
    public static function ensure_tables_exist() {
        ...
        if ( '' === (string) $current_version ) {
            $table = $wpdb->prefix . 'bizcity_webchat_messages';
            $exists = function_exists( 'bizcity_webchat_table_exists' )
                ? bizcity_webchat_table_exists( $table )
                : ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table ) ) === $table );
            $db->create_tables();
            if ( $exists ) {
                // Tables exist but no version tracked — run the column migrations too.
    ...
    private static $table_policy = array(
        ...
        // [2026-09-24 Claude Opus 5.5] WP-11 C1a — installed: messages + sources (live). Notes: no reader/writer.
    ...
    public function maybe_upgrade_conversations() {
        global $wpdb;
        $table_messages = $wpdb->prefix . 'bizcity_webchat_messages';
        // [2026-09-24 Claude Opus 5.5] CORE-REDUCTION WP-11 C0a — was used below (notes/sources CREATE) without
        // being set: those tables got the server default charset and PHP raised an undefined-variable warning.
        $charset_collate = $wpdb->get_charset_collate();
        ...
        // Migration 10 (v4.0.0 — Phase 1.8): webchat_notes table — [2026-09-24 Claude Opus 5.5] CORE-REDUCTION
        // WP-11 C1a: no longer created. Nothing reads or writes it (notes moved to the modules.twinchat.memory_notes
        // filestore, PHASE-1.30 §0.2.22; only BizCity_Memory_Table_Migration maps the legacy name). Existing
        // tables are left untouched; dropping them belongs to the legacy-table lifecycle.
        if ( false ) {
