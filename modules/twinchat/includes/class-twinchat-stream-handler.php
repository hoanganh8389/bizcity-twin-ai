...
class BizCity_TwinChat_Stream_Handler {
	...
	public function handle( array $args ) {
		...
		// Phase 0.12 Wave B+ — start Twin Event Stream turn (visualization spine).
	...
	private function handle_via_twin_agent( array $args, $user_id, $notebook_id, $session_id ) {
		...
		//   (3) notebook.character_id default            → sticky_source='default'
		...
		$_guru_class_ok = class_exists( 'BizCity_Twin_Guru_Context' ) && class_exists( 'BizCity_KG_Notebook_Service' );
		if ( ! $_guru_class_ok ) {
			error_log( sprintf(
				'[TwinChat][guru_lookup] SKIPPED — required classes missing: BizCity_Twin_Guru_Context=%s BizCity_KG_Notebook_Service=%s notebook_id=%d',
				class_exists( 'BizCity_Twin_Guru_Context' )    ? 'yes' : 'NO',
		...
		if ( $notebook_id > 0 && $_guru_class_ok ) {
			...
			if ( is_array( $args['target_guru'] ) && ! empty( $args['target_guru']['character_id'] ) ) {
				$_character_id  = (int) $args['target_guru']['character_id'];
			...
			if ( $_character_id <= 0 ) {
				$_sticky = get_user_meta( $user_id, 'bizcity_twin_sticky_guru_' . $notebook_id, true );
				if ( is_array( $_sticky ) && ! empty( $_sticky['character_id'] ) ) {
					$_character_id  = (int) $_sticky['character_id'];
			...
			error_log( sprintf(
				'[TwinChat][guru_lookup] notebook_id=%d user_id=%d character_id=%d sticky_source=%s mention=%s',
				$notebook_id, $user_id, $_character_id, $_sticky_source,
				( is_array( $args['target_guru'] ) && ! empty( $args['target_guru']['character_id'] ) ) ? 'yes' : 'no'
			) );
		...
		try {
			...
			if ( is_array( $_skill_match ) && ! empty( $_skill_match['tools'] ) ) {
				...
				if ( class_exists( 'BizCity_Twin_Tool_Registry' ) ) {
					$_reg = BizCity_Twin_Tool_Registry::instance();
					if ( method_exists( $_reg, 'get_all' ) ) {
						$_registry_keys = array_keys( (array) $_reg->get_all() );
					}
				}
