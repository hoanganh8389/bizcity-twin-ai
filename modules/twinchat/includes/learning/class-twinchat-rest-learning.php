...
class BizCity_TwinChat_REST_Learning {
	...
	/**
	 * Resolve TC-Learning daily log path (multisite-aware per uploads basedir).
	 */
	private function resolve_debug_log_path( $date_ymd = '', $ensure_dir = false ) {
		if ( function_exists( 'bizcity_tc_learning_debug_log_path' ) ) {
			return bizcity_tc_learning_debug_log_path( $date_ymd, (bool) $ensure_dir );
		}
		...
		$log_dir = trailingslashit( wp_normalize_path( $base_dir ) ) . 'bizcity-logs/tc-learning';
		...
		return trailingslashit( $log_dir ) . 'tc-learning-' . $date_ymd . '.log';
	}

	/**
	 * Read the tail of the dedicated learning debug log file.
	 * Path: uploads/site-basedir/bizcity-logs/tc-learning/tc-learning-YYYY-MM-DD.log
	 */
	public function read_debug_log( WP_REST_Request $req ) {
		$lines = max( 1, min( 2000, (int) $req->get_param( 'lines' ) ?: 200 ) );
		$date_ymd = (string) $req->get_param( 'date' );
		...
		if ( ! file_exists( $path ) ) {
			return rest_ensure_response( [
				'ok'    => true,
				'data'  => [ 'path' => $path, 'date' => $date_ymd, 'exists' => false, 'lines' => [] ],
			] );
		}
