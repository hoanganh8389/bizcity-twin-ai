...
// [2026-08-25 Johnny Chu] PHASE-1.29-SAFE-LOADER — degrade missing WebChat artifacts without a bootstrap fatal.
if ( ! class_exists( 'BizCity_Safe_Loader', false ) ) {
    $_webchat_root = defined( 'BIZCITY_TWIN_AI_DIR' ) ? BIZCITY_TWIN_AI_DIR : dirname( __DIR__, 2 ) . '/';
    $_webchat_safe_loader = $_webchat_root . 'core/helper/class-bizcity-safe-loader.php';
    if ( is_file( $_webchat_safe_loader ) && is_readable( $_webchat_safe_loader ) ) {
        require_once $_webchat_safe_loader;
    }
...
if ( ! function_exists( 'bizcity_webchat_require_safe' ) ) {
    function bizcity_webchat_require_safe( $path, $label = 'webchat.artifact' ) {
        return class_exists( 'BizCity_Safe_Loader', false )
            ? BizCity_Safe_Loader::require_file( $path, $label )
            : false;
    }
}

// Translations — load .po files from /languages/ (text domain: bizcity-twin-ai)
bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-webchat-widget.php', 'webchat.widget' );
bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-chatbot-shortcode.php', 'webchat.shortcode' );
bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-auth-ajax.php', 'webchat.auth_ajax' );
bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-login-page.php', 'webchat.login_page' );
bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'functions.php', 'webchat.functions' );
bizcity_webchat_require_safe( BIZCITY_WEBCHAT_LIB . 'class-webchat-ai.php', 'webchat.ai' );
...
add_action( 'init', function() {
    ...
    register_post_type( 'bizcity_mem_draft', array(
        'labels'          => array(
            'name'          => 'Memory Drafts',
...
// Always load — float widget, shortcodes, login page (needed on every frontend page)
    ...
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-webchat-trigger.php', 'webchat.trigger' );
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-webchat-timeline.php', 'webchat.timeline' );
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-webchat-memory.php', 'webchat.memory' );
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-webchat-api.php', 'webchat.api' );
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-admin-menu.php', 'webchat.admin_menu' );
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-admin-dashboard.php', 'webchat.admin_dashboard' );
    ...
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-ajax-handlers.php', 'webchat.ajax_handlers' );
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-plugin-suggestion-api.php', 'webchat.suggestions' );
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-plugin-gathering.php', 'webchat.gathering' );
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-automation-provider.php', 'webchat.automation_provider' );
    bizcity_webchat_require_safe( BIZCITY_WEBCHAT_INCLUDES . 'class-session-memory-spec.php', 'webchat.session_memory_spec' );
}
...
function bizcity_upload_to_media_library( $file ) {
    bizcity_webchat_require_safe( ABSPATH . 'wp-admin/includes/file.php', 'wordpress.upload_file' );
    bizcity_webchat_require_safe( ABSPATH . 'wp-admin/includes/image.php', 'wordpress.upload_image' );
    bizcity_webchat_require_safe( ABSPATH . 'wp-admin/includes/media.php', 'wordpress.upload_media' );
