...
class BizCity_Maturity_Calculator {
	...
	public static function ensure_table(): void {
		...
		dbDelta( "CREATE TABLE {$agg_table} (
			...
			execution_avg_goal_progress  DECIMAL(6,4) DEFAULT 100.0,
	...
	private static function get_detail_sources( int $uid, int $limit, int $offset ): array {
		...
		$rows = $wpdb->get_results( $wpdb->prepare(
			"SELECT id, title, source_type, source_url, status, char_count, chunk_count,
				token_estimate, project_id, created_at
			 FROM {$p}webchat_sources WHERE user_id = %d
		...
		$types = $wpdb->get_results( $wpdb->prepare(
			"SELECT COALESCE(source_type, 'unknown') AS label, COUNT(*) AS cnt
			 FROM {$p}webchat_sources WHERE user_id = %d GROUP BY source_type ORDER BY cnt DESC",
			$uid
		), ARRAY_A ) ?: [];
	...
	private static function get_detail_quickfaq( int $uid, int $limit, int $offset ): array {
		...
		$rows = $wpdb->get_results( $wpdb->prepare(
			"SELECT id, title, content, source_type, status, chunks_count, scope, created_at, updated_at
		...
		switch ( $tab ) {
			...
			case 'quickfaq':
				...
				if ( $action_type === 'delete' && $item_id ) {
					...
					$data = [
						'title'        => $question ?: mb_substr( $answer, 0, 80, 'UTF-8' ),
			...
			switch ( $tab ) {
				case 'quickfaq':
					...
					$wpdb->insert( $p . 'knowledge_sources', [
						'user_id' => $user_id, 'title' => $q ?: mb_substr( $a, 0, 80, 'UTF-8' ),
						'content' => $cj, 'content_hash' => md5( $cj ),
						'source_type' => 'quick_faq', 'status' => 'ready',
					] );
					break;
				case 'memories':
