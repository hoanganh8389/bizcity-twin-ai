...
class BizCity_Focus_Router {
    ...
    private static function is_astro_topic( string $message ): bool {
        $patterns = [
            ...
            // Future-oriented personal queries → cần bản đồ sao
            '/hôm nay.{0,20}(?:tôi|mình|em|anh|chị)\s+(?:thế nào|ra sao|nên)/ui',
            '/(?:ngày mai|tuần (?:này|sau|tới)|tháng (?:này|sau|tới)|năm (?:nay|tới|sau)).{0,30}(?:thế nào|ra sao|như thế nào|nên)/ui',
            '/(?:vận mệnh|dự báo|vận hạn|tương lai).{0,30}(?:tài chính|sự nghiệp|tình cảm|sức khỏe|công việc|tình duyên|hôn nhân)/ui',
            '/(?:tình hình|xu hướng).{0,20}(?:tài chính|sự nghiệp|tình cảm|sức khỏe).{0,20}(?:tương lai|sắp tới|tới đây)/ui',
        ];
