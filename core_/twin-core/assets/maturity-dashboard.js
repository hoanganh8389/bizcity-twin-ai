...
(function() {
	...
	var TAB_COLUMNS = {
		...
		sources: {
			...
			cols: [
				{ f: 'title', l: 'Tiêu đề', w: true, get: function(r) { return r.title || r.source_url || '—'; } },
				{ f: 'source_type', l: 'Loại', badge: true },
				{ f: 'status', l: 'TT', badge: true },
				{ f: 'char_count', l: 'Ký tự' },
				{ f: 'chunk_count', l: 'Chunks' },
				{ f: 'created_at', l: 'Ngày', dt: true },
			],
		},
		sessions: {
	...
	function renderEditableTable(tab, items) {
		...
		if (cfg.editable) html += '<th class="bk-col-action"></th>';
		html += '</tr></thead><tbody>';

		if (!items.length) {
			var emptyColspan = cols.length + 1 + (cfg.editable ? 1 : 0);
			html += '<tr class="bk-empty-row"><td colspan="' + emptyColspan + '" style="text-align:center;color:#9ca3af;padding:24px 0">' + (cfg.editable ? 'Chưa có dữ liệu. Nhấn <strong>+ Thêm</strong> để tạo.' : 'Chưa có dữ liệu.') + '</td></tr>';
		}
